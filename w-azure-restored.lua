-- ===========================================================================
--  W-AZURE V2 • RESTORED & UPGRADED BUILD
--  Base      : realw-azure (2023)
--  Features  : stolen / ported from DIO Hub
--  ------------------------------------------------------------------------
--  Da sua:
--    * 12 loi cu phap o block "Auto Set Spawn Point"
--    * createKeybind crash + leak InputBegan
--    * RepStorage nil (Auto Legendary Sword)
--    * getupvalues crash tren executor khong ho tro
--    * loadstring remote khong duoc bao ve (SmoothScroll + Script6)
--    * _G.Settings ghi de config
--  Da khoi phuc:
--    * Toan bo menu W-Azure (ban 2023 chi con do-block demo)
--  Da bo sung:
--    * No Clip loop (engine chi set _G.Noclip, khong co loop tieu thu)
--    * Auto Store All Fruits (setting co san nhung khong co code)
--    * Auto Haki / Ken / Auto Stats / ESP / Redeem Codes / Teleport
--    * Config save-load (writefile) + hotkey an UI
-- ===========================================================================
...
-- [W-Azure Restored] loadstring SmoothScroll khong duoc bao ve -> URL chet thi
-- toan bo script dung va KHONG CO MENU. Toan bo utils.smoothscrolling.* trong
-- source deu da bi comment nen thay bang stub an toan + fallback remote.
utils.smoothscrolling = (function()
    local stub = { Enable = function() end, Disable = function() end }
    local okLoad, src = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/lug1adev/W-Azure/main/SmoothScroll.lua", true)
    end)
    if okLoad and type(src) == "string" and #src > 0 and loadstring then
        local okFn, mod = pcall(function()
            local fn = loadstring(src)
            if fn then return fn() end
        end)
        if okFn and type(mod) == "table" then return mod end
    end
    return stub
end)()
        ...
        WindowFunctions.createTab = function(tabName, defaultTab)
            ...
            TabFunctions.createKeybind = function(name, icon, default, callback)
                ...
                local StartBinding = function()
                    ...
                    -- [W-Azure Restored] disconnect sau khi bind xong (ban cu leak connection
                    -- va goi callback lien tuc cho moi phim bam)
                    local keybindConn
                    keybindConn = UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
                            ...
                            if bindingInProgress then
                                if table.find(Blacklisted, input.KeyCode) then
                                    if table.find(keys, input.KeyCode) then
                                        KeybindTextLabel.Text = keys[input.KeyCode]
                                        boundKey = previousKey
                                        pcall(callback, Enum.KeyCode[previousKey])
                                    else
...
-- ============================================================================
--  W-AZURE V2 • BLOX FRUITS MENU  (RESTORED + UPGRADED)
--  ------------------------------------------------------------------
--  Phan menu nay bi mat trong ban 2023 (chi con lai do-block demo).
--  Da duoc khoi phuc lai theo dung API cua library W-Azure V2 va
--  bo sung chuc nang lay tu DIO Hub (island table, store fruit, haki,
--  stats, ESP, codes, ...).
-- ============================================================================
do
    ----------------------------------------------------------------------------
    -- 0. DEFAULT SETTINGS (phai chay TRUOC khi engine y() va truoc khi tao menu)
    ----------------------------------------------------------------------------
    local WA_DEFAULTS = {
        ["Selected_Weapons"]            = "Melee",
        ["Auto_Farm_Level"]             = false,
        ["Auto_Farm_Saber"]             = false,
        ["Auto_Legendary_Sword"]        = false,
        ["Auto_Legendary_Sword_Hop"]    = false,
        ["Auto_New_World"]              = true,
        ["Auto_Third_Sea"]              = true,
        ["Grab_All_Fruits"]             = false,
        ["Grab_All_Fruits_Hop"]         = false,
        ["Grab_All_Fruits_Hop_Level"]   = 1500,
        ["Auto_Store_All_Fruits"]       = false,
        ["Auto_Eat_Fruits"]             = false,
        ["Auto_Eat_List"] = {
            'Dark-Dark', 'Light-Light', 'Quake-Quake', 'Human-Human: Buddha',
            'String-String', 'Bird-Bird: Phoenix', 'Rumble-Rumble', 'Paw-Paw',
            'Gravity-Gravity', 'Dough-Dough', 'Shadow-Shadow', 'Venom-Venom',
            'Control-Control', 'Soul-Soul', 'Dragon-Dragon', 'Spirit-Spirit',
            'Gas-Gas', 'Sound-Sound', 'Kitsune-Kitsune', 'Leopard-Leopard',
        },
    }

    -- Cac bien dieu khien bo sung (engine + menu dung chung)
    local WA_EXTRA = {
        WA_FastAttack   = true,
        WA_TweenSpeed   = 300,
        WA_AutoHaki     = true,
    ...
    for _k, _v in pairs(WA_DEFAULTS) do
        if _G.Settings[_k] == nil then
            if type(_v) == "table" then
                local _copy = {}
                for _i, _x in ipairs(_v) do _copy[_i] = _x end
                _G.Settings[_k] = _copy
            else
                _G.Settings[_k] = _v
            end
        end
    end
    for _k, _v in pairs(WA_EXTRA) do
        if _G[_k] == nil then _G[_k] = _v end
    end
    _G.Noclip          = _G.Noclip or false
    _G.BreakAllProcess = _G.BreakAllProcess or false
    _G.TP_SETSPAWN     = (_G.TP_SETSPAWN == nil) and true or _G.TP_SETSPAWN
    if _G.Teams == nil then _G.Teams = "Pirates" end
    ...
    local function WA_Notify(text, dur)
        if _G.WA_Notify == false then return end
        pcall(function() library.notify("[W-Azure] " .. tostring(text), dur or 3) end)
    end
    WA.Notify = WA_Notify

    -- Remote accessor an toan (Remotes co the chua ton tai luc script chay)
    ...
    local function CommE()
        local r = ReplicatedStorage:FindFirstChild("Remotes")
        return r and r:FindFirstChild("CommE")
    end
    WA.CommF, WA.CommE = CommF, CommE
    ...
    local function Invoke(name, ...)
        ...
        local ok, res = pcall(function()
            local rf = CommF()
            if rf then return rf:InvokeServer(name, t_unpack(args, 1, args.n)) end
        end)
    ...
    local function Fire(name, ...)
        ...
        pcall(function()
            local rf = CommE()
            if rf then rf:FireServer(name, t_unpack(args, 1, args.n)) end
        end)
    end
    WA.Invoke, WA.Fire = Invoke, Fire
    ...
    local function Data(key)
        ...
        return d and d:FindFirstChild(key)
    end
    WA.Data = Data

    local function World()
        local id = game.PlaceId
        if id == 2753915549 then return 1
        elseif id == 4442272183 then return 2
        elseif id == 7449423635 then return 3 end
        return 0
    end
    WA.World = World

    -- Tween toi CFrame (viet moi, khong phu thuoc tween cua DIO)
    local WA_TweenConn = nil
    local function TweenTo(cf, speed)
        speed = tonumber(speed) or tonumber(_G.WA_TweenSpeed) or 300
        local r = Root()
        if not r or not cf then return false end
        pcall(function()
            if WA_TweenConn then WA_TweenConn:Disconnect() WA_TweenConn = nil end
        end)
        local dist = (r.Position - cf.Position).Magnitude
        local t = math.clamp(dist / speed, 0.15, 30)
        local ok = pcall(function()
            local tw = TweenService:Create(r, TweenInfo.new(t, Enum.EasingStyle.Linear), { CFrame = cf })
            WA_TweenConn = tw.Completed:Connect(function()
                pcall(function() WA_TweenConn:Disconnect() end)
                WA_TweenConn = nil
            end)
            tw:Play()
        end)
    ...
    WA.StopTween = StopTween

    -- File helper (co the khong ton tai tren mot so executor)
    local function WA_Write(path, txt)
        local ok = pcall(function()
            if writefile then writefile(path, txt) end
        end)
        return ok
    end
    local function WA_Read(path)
        local ok, res = pcall(function()
    ...
    ----------------------------------------------------------------------------
    -- 2. DATA TABLES  (steal tu DIO Hub)
    ----------------------------------------------------------------------------
    local Sea1_Islands = {
        ["Pirates"]         = { CFrame = CFrame.new(-2709.68, 24.52, 2104.25),  Mobs = { "Bandit" } },
        ["Marine"]          = { CFrame = CFrame.new(-2510, 45, 2000),           Mobs = { "Trainee" } },
    ...
    local Sea2_Islands = {
        ...
        ["Hot and Cold (Cold)"] = { CFrame = CFrame.new(-6026, 15, -5062),   Mobs = { "Lab Subordinate", "Horned Warrior" } },
        ["Hot and Cold (Hot)"]  = { CFrame = CFrame.new(-5478, 15, -5240),   Mobs = { "Magma Ninja", "Lava Pirate" } },
        ["Cursed Ship"]         = { CFrame = CFrame.new(902, 126, 33071),    Mobs = { "Ship Deckhand", "Ship Engineer", "Ship Steward", "Ship Officer" } },
        ["Ice Castle"]          = { CFrame = CFrame.new(6137, 294, -6747),   Mobs = { "Arctic Warrior", "Snow Lurker" } },
        ["Forgotten Island"]    = { CFrame = CFrame.new(-3043, 238, -10191), Mobs = { "Sea Soldier", "Water Fighter" } },
    }
    local Sea3_Islands = {
        ["Port Town"]        = { CFrame = CFrame.new(-290, 44, 5450),     Mobs = { "Pirate Millionaire", "Pistol Billionaire" } },
        ["Hydra Island"]     = { CFrame = CFrame.new(5228, 604, 345),     Mobs = { "Dragon Crew Warrior", "Dragon Crew Archer", "Female Islander", "Giant Islander" } },
        ["Great Tree"]       = { CFrame = CFrame.new(2682, 1682, -7190),  Mobs = { "Marine Commodore", "Marine Rear Admiral" } },
        ["Floating Turtle"]  = { CFrame = CFrame.new(-12000, 331, -8500), Mobs = { "Forest Pirate", "Mythological Pirate", "Jungle Pirate", "Musketeer Pirate", "Fishman Raider", "Fishman Captain" } },
        ["Haunted Castle"]   = { CFrame = CFrame.new(-9515, 142, 5536),   Mobs = { "Reborn Skeleton", "Living Zombie", "Demonic Soul", "Posessed Mummy" } },
        ["Sea of Treats"]    = { CFrame = CFrame.new(-1145, 13, -14450),  Mobs = { "Peanut Scout", "Peanut President", "Ice Cream Commander", "Cookie Crafter", "Cake Guard", "Baking Staff", "Head Baker", "Cocoa Warrior", "Chocolate Bar Battler", "Sweet Thief", "Candy Rebel" } },
        ["Tiki Outpost"]     = { CFrame = CFrame.new(-16200, 90, -17300), Mobs = { "Isle Outlaw", "Island Boy", "Sun-kissed Warrior", "Isle Champion" } },
        ["Submerged Island"] = { CFrame = CFrame.new(-3200, -10, -10000), Mobs = { "Reef Bandit", "Coral Pirate", "Sea Chanter", "Ocean Prophet" } },
    }
    local IslandSets = { [1] = Sea1_Islands, [2] = Sea2_Islands, [3] = Sea3_Islands }
    WA.Islands = IslandSets

    local function IslandNames()
        local set = IslandSets[World()] or Sea1_Islands
        local out = {}
    ...
    WA.IslandNames, WA.MobNames = IslandNames, MobNames

    local BossList = {
        [1] = { "The Gorilla King", "Bobby", "The Saw", "Yeti", "Mob Leader", "Vice Admiral", "Saber Expert",
                "Warden", "Chief Warden", "Swan", "Magma Admiral", "Fishman Lord", "Wysper", "Thunder God",
                "Cyborg", "Ice Admiral", "Greybeard" },
        [2] = { "Diamond", "Jeremy", "Orbitus", "Don Swan", "Smoke Admiral", "Awakened Ice Admiral",
                "Tide Keeper", "Darkbeard", "Cursed Captain", "Order" },
        [3] = { "Stone", "Hydra Leader", "Kilo Admiral", "Captain Elephant", "Beautiful Pirate", "Cake Queen",
                "Dough King", "Longma", "Soul Reaper", "rip_indra True Form", "Tyrant of the Skies" },
    }
    WA.BossList = BossList
    ...
    local MaterialList = {
        ...
        [3] = { "Scrap Metal", "Demonic Wisp", "Conjured Cocoa", "Dragon Scale", "Gunpowder", "Fish Tail", "Mini Tusk" },
    }
    WA.MaterialList = MaterialList
    ...
    local FruitList = {
        ...
        "Dough-Dough", "Shadow-Shadow", "Venom-Venom", "Control-Control", "Soul-Soul",
        "Spirit-Spirit", "Dragon-Dragon", "Leopard-Leopard", "Gas-Gas", "Sound-Sound",
        "Kitsune-Kitsune",
    }
    WA.FruitList = FruitList

    local WeaponList = { "Melee", "Sword", "Blox Fruit", "Gun" }

    local CodeList = {
        "Redeem All Codes", "TheGreatAce", "Bignews", "Fudd10", "Sub2OfficialNoobie", "StrawHatMaine",
        "TantaiGaming", "Axiore", "Sub2Daigrock", "Sub2NoobMaster123", "SUB2GAMERROBOT_EXP1",
        "UPD16", "3BVISITS", "fudd10_v2", "Bluxxy", "JCWK", "Starcodeheo", "Magicbus",
        "Enyu_is_Pro", "Sub2Fer999", "SUB2GAMERROBOT_RESET1", "KittGaming", "Sub2CaptainMaui",
        "Starcodeheo", "Enyu_is_Pro", "Axiore", "Magicbus", "Bluxxy", "Fudd10_v2",
    }
    WA.CodeList = CodeList

    ----------------------------------------------------------------------------
    -- 3. FEATURE FUNCTIONS  (khoi phuc + steal tu DIO)
    ----------------------------------------------------------------------------
    -- 3.1 Store all devil fruits  (DIO: UpdStFruit)
    function WA.StoreAllFruits()
        local n = 0
        pcall(function()
            ...
            for _, tool in ipairs(bp:GetChildren()) do
                pcall(function()
                    local eat = tool:FindFirstChild("EatRemote", true)
                    if eat then
                        Invoke("StoreFruit", tool:GetAttribute("OriginalName") or tool.Name, tool)
                        n = n + 1
                    end
                end)
            end
    ...
    -- 3.2 Grab nearest devil fruit in workspace
    function WA.GrabNearestFruit()
        local r = Root()
        ...
        pcall(function()
            for _, v in ipairs(workspace:GetChildren()) do
                if string.find(v.Name, "Fruit") then
                    ...
                    if handle then
                        local d = (handle.Position - r.Position).Magnitude
                        if d < bestDist then bestDist = d best = handle end
                    end
                end
            end
        end)
        if best then
            TweenTo(best.CFrame)
            task.wait(math.clamp(bestDist / math.max(tonumber(_G.WA_TweenSpeed) or 300, 1), 0.2, 30))
            pcall(function() best.CFrame = r.CFrame end)
            return true
        end
        return false
    end

    -- 3.3 Haki / Buso  (DIO: AutoHaki)
    function WA.Haki()
        pcall(function()
            local c = plr.Character
    ...
    -- 3.4 Observation / Ken  (DIO: CommE Ken)
    function WA.Ken()
        Fire("Ken", true)
    end

    -- 3.5 Stat points  (DIO: statsSetings)
    local StatRemoteName = {
        Melee = "Melee", Defense = "Defense", Sword = "Sword",
        Gun = "Gun", ["Devil Fruit"] = "Demon Fruit",
    }
    function WA.AddStat(stat, amount)
        local rn = StatRemoteName[stat]
        if not rn then return false end
        amount = tonumber(amount) or 1
        local ok = pcall(function()
            local pts = plr.Data and plr.Data:FindFirstChild("Points")
            if pts and tonumber(pts.Value) and tonumber(pts.Value) > 0 then
                Invoke("AddPoint", rn, amount)
            end
        end)
        return ok
    end
    ...
    -- 3.6 Redeem codes
    function WA.RedeemAllCodes()
        local n = 0
        for _, code in ipairs(CodeList) do
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
                local red = r and r:FindFirstChild("Redeem")
                if red then red:InvokeServer(tostring(code)) n = n + 1 end
            end)
        end
        return n
    end

    -- 3.7 Legendary sword dealer
    function WA.LegendarySwordDealer()
        Invoke("LegendarySwordDealer")
        Invoke("LegendarySwordDealer", "1")
        Invoke("LegendarySwordDealer", "2")
    end

    -- 3.8 Team switch
    function WA.SetTeam(team)
        Invoke("SetTeam2", tostring(team))
    end

    -- 3.9 Hop server (dung lai TPReturner cua engine neu da ton tai)
    function WA.Hop()
        if type(getgenv().Teleport) == "function" then
            pcall(function() getgenv().Teleport() end)
            return true
        end
        WA_Notify("Engine chua san sang de hop server.", 4)
        return false
    end

    -- 3.10 ESP (viet moi - BillboardGui don gian, khong gioi han executor)
    local ESP_Holder = nil
    local function ESP_Clear()
        pcall(function() if ESP_Holder then ESP_Holder:Destroy() end end)
        ESP_Holder = nil
    end
    local function ESP_Make(target, text, color)
        pcall(function()
            ...
            lbl.TextColor3 = color or Color3.fromRGB(0, 179, 255)
    ...
    function WA.ESP(mode, on)
        pcall(function()
            local pg = plr:FindFirstChild("PlayerGui")
            if not pg then return end
            if mode == "Clear" then
                for _, d in ipairs(pg:GetDescendants()) do
                    if d.Name == "WA_ESP" then d:Destroy() end
                end
    ...
    ----------------------------------------------------------------------------
    -- 4. BACKGROUND LOOPS (chay doc lap voi engine y())
    ----------------------------------------------------------------------------
    task.spawn(function()
        while true do
            task.wait(0.6)
            pcall(function()
                if _G.WA_AutoHaki then WA.Haki() end
            end)
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(1)
            pcall(function()
                if _G.WA_AutoKen then WA.Ken() end
            end)
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(1.5)
            pcall(function()
                if _G.WA_AutoStat and _G.WA_AutoStat ~= "None" then
                    WA.AddStat(_G.WA_AutoStat, tonumber(_G.WA_StatAmount) or 1)
                end
            end)
        end
    end)
    ...
    task.spawn(function()
        while true do
            task.wait(4)
            pcall(function()
                if _G.Settings.Auto_Store_All_Fruits then
                    WA.StoreAllFruits()
                end
            end)
        end
    ...
    -- No Clip loop (engine chi set _G.Noclip, chua co vong lap tieu thu -> da bo sung)
    task.spawn(function()
    ...
    task.spawn(function()
        pcall(function()
            plr.Idled:Connect(function()
                if _G.WA_AntiAFK == false then return end
                pcall(function()
    ...
    task.spawn(function()
        while true do
            task.wait(3)
            pcall(function()
                if not _G.WA_ESP_Players then
                    for _, p in ipairs(Players:GetPlayers()) do
                        if p ~= plr and p.Character then
                            local bb = p.Character:FindFirstChild("WA_ESP", true)
                ...
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= plr and p.Character then
                        if not p.Character:FindFirstChild("WA_ESP", true) then
                            local lvl = "?"
                            pcall(function() lvl = tostring(p.Data.Level.Value) end)
                            ESP_Make(p.Character, p.Name .. " [" .. lvl .. "]", Color3.fromRGB(255, 90, 90))
                        end
                    end
                end
            end)
    ...
    -- ESP devil fruit loop
    task.spawn(function()
        while true do
            task.wait(3)
            pcall(function()
                if not _G.WA_ESP_Devil then
                    for _, d in ipairs(workspace:GetDescendants()) do
                        if d.Name == "WA_ESP" and d.Parent and d.Parent.Name == "Handle" then
                            d:Destroy()
                        end
                    end
                    return
                ...
                for _, v in ipairs(workspace:GetChildren()) do
                    if string.find(v.Name, "Fruit") then
                        local handle = v:FindFirstChild("Handle")
                        if handle and not handle:FindFirstChild("WA_ESP") then
                            local bb = ESP_Make(handle, v.Name, Color3.fromRGB(255, 190, 60))
                            pcall(function() if bb then bb.Parent = handle end end)
                        end
                    end
                end
            end)
    ...
    task.spawn(function()
        while true do
            ...
            pcall(function()
                if _G.WA_NoFog then
                    if _savedFog == nil then
                        _savedFog = { Lighting.FogStart, Lighting.FogEnd }
                    end
                    Lighting.FogStart = 0
                    Lighting.FogEnd = 100000
                elseif _savedFog ~= nil then
                    Lighting.FogStart = _savedFog[1]
                    Lighting.FogEnd = _savedFog[2]
                    _savedFog = nil
                end
            end)
        end
    ...
    -- 5. CONFIG SAVE / LOAD
    ----------------------------------------------------------------------------
    local CONFIG_PATH = "wazure_config.json"
    function WA.SaveConfig()
        local payload = { Settings = _G.Settings, Extra = {} }
        for k in pairs(WA_EXTRA) do
            payload.Extra[k] = _G[k]
        end
        payload.Extra.Noclip = _G.Noclip
        payload.Extra.TP_SETSPAWN = _G.TP_SETSPAWN
        local ok, enc = pcall(function()
            return game:GetService("HttpService"):JSONEncode(payload)
        end)
        if not ok then return false, "Encode fail" end
        if not WA_Write(CONFIG_PATH, enc) then return false, "writefile khong kha dung" end
        return true
    end
    function WA.LoadConfig()
        local raw = WA_Read(CONFIG_PATH)
        if not raw then return false, "Chua co config" end
        local ok, dec = pcall(function()
            return game:GetService("HttpService"):JSONDecode(raw)
        end)
        if not ok or type(dec) ~= "table" then return false, "Config hong" end
        if type(dec.Settings) == "table" then
            for k, v in pairs(dec.Settings) do _G.Settings[k] = v end
        end
        if type(dec.Extra) == "table" then
            for k, v in pairs(dec.Extra) do _G[k] = v end
        end
        return true
    end
    ...
    ----------------------------------------------------------------------------
    local IC = {
        sword   = "rbxassetid://13286125855",
        star    = "rbxassetid://13286125855",
        bolt    = "rbxassetid://13286125855",
        gear    = "rbxassetid://13286125855",
        map     = "rbxassetid://13286125855",
        fruit   = "rbxassetid://13286125855",
        shield  = "rbxassetid://13286125855",
        chart   = "rbxassetid://13286125855",
    }
    ...
    local okWindow, Window = pcall(function()
        return library.createWindow(true)
    end)

    if not okWindow or type(Window) ~= "table" or type(Window.createTab) ~= "function" then
        WA_Notify("Khong tao duoc cua so W-Azure. Vui long rejoin game.", 8)
    else

    ------------------------------------------------------------------ TAB: Main
    local Main = Window.createTab("Main", true)
    Main.createTabHeader("W-Azure Restored", "Blox Fruits • base 2023 + DIO features")

    local InfoLabel = Main.createLabel("Status", IC.chart)

    task.spawn(function()
        while true do
            task.wait(1)
            pcall(function()
                local lvl, fruit, w = "?", "None", World()
                local d = plr:FindFirstChild("Data")
                if d then
                    if d:FindFirstChild("Level") then lvl = tostring(d.Level.Value) end
                    if d:FindFirstChild("DevilFruit") then fruit = tostring(d.DevilFruit.Value) end
                end
                if InfoLabel and InfoLabel.Update then
                    InfoLabel.Update(string.format("Level: %s  |  World: %d  |  Fruit: %s", lvl, w, fruit))
                end
            end)
        end
    end)

    Main.createToggle("Auto Farm Level", IC.sword, _G.Settings.Auto_Farm_Level, function(v)
        _G.Settings.Auto_Farm_Level = v
        if not v then _G.BreakAllProcess = false end
        WA_Notify("Auto Farm Level: " .. tostring(v))
    end)

    Main.createToggle("Fast Attack", IC.bolt, _G.WA_FastAttack, function(v)
        _G.WA_FastAttack = v
    end)

    Main.createToggle("Auto Farm Saber", IC.sword, _G.Settings.Auto_Farm_Saber, function(v)
        _G.Settings.Auto_Farm_Saber = v
    end)

    Main.createToggle("Auto Legendary Sword", IC.star, _G.Settings.Auto_Legendary_Sword, function(v)
        _G.Settings.Auto_Legendary_Sword = v
    end)

    Main.createToggle("Auto Legendary Sword + Hop", IC.star, _G.Settings.Auto_Legendary_Sword_Hop, function(v)
        _G.Settings.Auto_Legendary_Sword_Hop = v
    end)

    Main.createToggle("Auto New World", IC.map, _G.Settings.Auto_New_World, function(v)
        _G.Settings.Auto_New_World = v
    end)

    Main.createToggle("Auto Third Sea", IC.map, _G.Settings.Auto_Third_Sea, function(v)
        _G.Settings.Auto_Third_Sea = v
    end)

    Main.createDropdown("Select Weapon", IC.sword, false, WeaponList, _G.Settings.Selected_Weapons, function(v)
        if v then _G.Settings.Selected_Weapons = v end
    end, 4)

    Main.createSlider("Tween Speed", IC.bolt, 50, 600, tonumber(_G.WA_TweenSpeed) or 300, function(v)
        _G.WA_TweenSpeed = tonumber(v) or 300
    end)

    Main.createToggle("No Clip", IC.shield, _G.Noclip, function(v)
        _G.Noclip = v
    end)

    Main.createToggle("Auto Haki (Buso)", IC.shield, _G.WA_AutoHaki, function(v)
        _G.WA_AutoHaki = v
        if v then WA.Haki() end
    end)

    Main.createToggle("Auto Ken (Observation)", IC.shield, _G.WA_AutoKen, function(v)
        _G.WA_AutoKen = v
    end)

    Main.createToggle("Auto Set Spawn Point", IC.map, _G.TP_SETSPAWN, function(v)
        _G.TP_SETSPAWN = v
        if v then Invoke("SetSpawnPoint") end
    end)

    Main.createToggle("Anti AFK", IC.gear, _G.WA_AntiAFK, function(v)
        _G.WA_AntiAFK = v
    end)

    Main.createButton("Stop All Processes", IC.gear, function()
        _G.BreakAllProcess = true
        _G.Settings.Auto_Farm_Level = false
        StopTween()
        WA_Notify("Da dung toan bo process.")
    end)

    Main.createKeybind("Toggle UI Key", IC.gear, Enum.KeyCode.RightControl, function(key)
        getgenv().WA_UIKey = key
    end)

    ---------------------------------------------------------------- TAB: Fruits
    local Fruit = Window.createTab("Fruits", false)
    Fruit.createTabHeader("Devil Fruit", "Eat / Store / Grab")

    Fruit.createToggle("Auto Eat Fruits", IC.fruit, _G.Settings.Auto_Eat_Fruits, function(v)
        _G.Settings.Auto_Eat_Fruits = v
    end)

    Fruit.createDropdown("Auto Eat List", IC.fruit, {}, FruitList, _G.Settings.Auto_Eat_List, function(v)
        if type(v) == "table" then
            _G.Settings.Auto_Eat_List = v
        end
    end, 6)

    Fruit.createToggle("Auto Store All Fruits", IC.fruit, _G.Settings.Auto_Store_All_Fruits, function(v)
        _G.Settings.Auto_Store_All_Fruits = v
        if v then
            local n = WA.StoreAllFruits()
            WA_Notify("Da store " .. n .. " trai.")
        end
    end)

    Fruit.createToggle("Grab All Fruits", IC.fruit, _G.Settings.Grab_All_Fruits, function(v)
        _G.Settings.Grab_All_Fruits = v
    end)

    Fruit.createToggle("Grab All Fruits + Hop", IC.fruit, _G.Settings.Grab_All_Fruits_Hop, function(v)
        _G.Settings.Grab_All_Fruits_Hop = v
    end)

    Fruit.createSlider("Hop Level Requirement", IC.chart, 1, 2550,
        tonumber(_G.Settings.Grab_All_Fruits_Hop_Level) or 1500, function(v)
        _G.Settings.Grab_All_Fruits_Hop_Level = tonumber(v) or 1500
    end)

    Fruit.createButton("Store All Fruits Now", IC.fruit, function()
        local n = WA.StoreAllFruits()
        WA_Notify("Da store " .. n .. " trai.")
    end)

    Fruit.createButton("Grab Nearest Fruit", IC.fruit, function()
        if WA.GrabNearestFruit() then
            WA_Notify("Da toi vi tri trai gan nhat.")
        else
            WA_Notify("Khong tim thay trai nao trong server.")
        end
    end)

    -------------------------------------------------------------- TAB: Teleport
    local Travel = Window.createTab("Teleport", false)
    Travel.createTabHeader("Teleport", "Island / Mob / Boss")

    local IslandDD = Travel.createDropdown("Select Island", IC.map, false, IslandNames(), IslandNames()[1], function(v)
        _G.WA_SelectIsland = v
    end, 5)

    local MobDD = Travel.createDropdown("Select Mob", IC.sword, false, MobNames(), MobNames()[1], function(v)
        _G.WA_SelectMob = v
    end, 5)

    local BossDD = Travel.createDropdown("Select Boss", IC.star, false, BossList[World()] or BossList[1], (BossList[World()] or BossList[1])[1], function(v)
        _G.WA_SelectBoss = v
    end, 5)

    Travel.createButton("Refresh World Lists", IC.map, function()
        pcall(function()
            IslandDD.Refresh(IslandNames(), IslandNames()[1])
            MobDD.Refresh(MobNames(), MobNames()[1])
            BossDD.Refresh(BossList[World()] or BossList[1], (BossList[World()] or BossList[1])[1])
        end)
        WA_Notify("Da cap nhat danh sach theo world hien tai.")
    end)

    Travel.createButton("Teleport To Island", IC.map, function()
        local set = IslandSets[World()] or Sea1_Islands
        local data = _G.WA_SelectIsland and set[_G.WA_SelectIsland]
        if data then
            TweenTo(data.CFrame)
            WA_Notify("Dang di chuyen toi " .. tostring(_G.WA_SelectIsland))
        else
            WA_Notify("Chon island truoc.")
        end
    end)

    Travel.createButton("Teleport To Mob", IC.sword, function()
        local target = nil
        pcall(function()
            local en = workspace:FindFirstChild("Enemies")
            if en then target = en:FindFirstChild(tostring(_G.WA_SelectMob)) end
            if not target then target = ReplicatedStorage:FindFirstChild(tostring(_G.WA_SelectMob)) end
        end)
        if target and target:FindFirstChild("HumanoidRootPart") then
            TweenTo(target.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
            WA_Notify("Dang di chuyen toi " .. tostring(_G.WA_SelectMob))
        else
            WA_Notify("Khong tim thay mob trong server.")
        end
    end)

    Travel.createButton("Teleport To Boss", IC.star, function()
        local target = nil
        pcall(function()
            local en = workspace:FindFirstChild("Enemies")
            if en then
                for _, m in ipairs(en:GetChildren()) do
                    if string.find(m.Name, tostring(_G.WA_SelectBoss)) then target = m break end
                end
            end
        end)
        if target and target:FindFirstChild("HumanoidRootPart") then
            TweenTo(target.HumanoidRootPart.CFrame * CFrame.new(0, 20, 0))
            WA_Notify("Dang di chuyen toi " .. tostring(_G.WA_SelectBoss))
        else
            WA_Notify("Khong tim thay boss trong server.")
        end
    end)

    Travel.createButton("Stop Tween", IC.gear, function()
        StopTween()
        WA_Notify("Da dung tween.")
    end)

    ------------------------------------------------------------------ TAB: Stats
    local Stats = Window.createTab("Stats", false)
    Stats.createTabHeader("Stats", "Auto / Manual point allocation")

    Stats.createDropdown("Auto Stat", IC.chart, false,
        { "None", "Melee", "Defense", "Sword", "Gun", "Devil Fruit" }, _G.WA_AutoStat, function(v)
        if v then _G.WA_AutoStat = v end
    end, 6)

    Stats.createSlider("Points Per Tick", IC.chart, 1, 50, tonumber(_G.WA_StatAmount) or 1, function(v)
        _G.WA_StatAmount = tonumber(v) or 1
    end)

    Stats.createButton("+1 Melee", IC.sword, function() WA.AddStat("Melee", 1) end)
    Stats.createButton("+1 Defense", IC.shield, function() WA.AddStat("Defense", 1) end)
    Stats.createButton("+1 Sword", IC.sword, function() WA.AddStat("Sword", 1) end)
    Stats.createButton("+1 Gun", IC.bolt, function() WA.AddStat("Gun", 1) end)
    Stats.createButton("+1 Devil Fruit", IC.fruit, function() WA.AddStat("Devil Fruit", 1) end)

    ------------------------------------------------------------------- TAB: Shop
    local Shop = Window.createTab("Shop", false)
    Shop.createTabHeader("Shop", "Haki / Sword / Team")

    Shop.createButton("Buy Haki (Buso)", IC.shield, function()
        Invoke("BuyHaki")
        WA.Haki()
        WA_Notify("Da gui yeu cau mua Haki.")
    end)

    Shop.createButton("Legendary Sword Dealer", IC.star, function()
        WA.LegendarySwordDealer()
        WA_Notify("Da goi Legendary Sword Dealer.")
    end)

    Shop.createButton("Join Pirates", IC.map, function()
        WA.SetTeam("Pirates")
        _G.Teams = "Pirates"
    end)

    Shop.createButton("Join Marines", IC.map, function()
        WA.SetTeam("Marines")
        _G.Teams = "Marines"
    end)

    Shop.createButton("Hop Server", IC.bolt, function()
        WA.Hop()
    end)

    ------------------------------------------------------------------- TAB: Misc
    local Misc = Window.createTab("Misc", false)
    Misc.createTabHeader("Misc", "ESP / Codes / Visual")

    Misc.createToggle("ESP Players", IC.chart, _G.WA_ESP_Players, function(v)
        _G.WA_ESP_Players = v
        if not v then WA.ESP("Clear") end
    end)

    Misc.createToggle("ESP Devil Fruits", IC.fruit, _G.WA_ESP_Devil, function(v)
        _G.WA_ESP_Devil = v
        if not v then WA.ESP("Clear") end
    end)

    Misc.createToggle("Remove Fog", IC.gear, _G.WA_NoFog, function(v)
        _G.WA_NoFog = v
    end)

    Misc.createToggle("Notifications", IC.gear, _G.WA_Notify, function(v)
        _G.WA_Notify = v
    end)

    Misc.createButton("Redeem All Codes", IC.star, function()
        local n = WA.RedeemAllCodes()
        WA_Notify("Da gui " .. n .. " code.")
    end)

    Misc.createButton("Clear ESP", IC.gear, function()
        WA.ESP("Clear")
        WA_Notify("Da xoa ESP.")
    end)

    --------------------------------------------------------------- TAB: Settings
    local Set = Window.createTab("Settings", false)
    Set.createTabHeader("Settings", "Config / Theme")

    local ThemeNames = { "LightBlue", "DarkBlue", "GreyDark" }
    Set.createDropdown("Theme (ap dung cho element moi)", IC.gear, false, ThemeNames,
        tostring(_G.WA_Theme), function(v)
        if v and getgenv().Theme and getgenv().Theme[v] then
            _G.WA_Theme = v
            local okT = pcall(function()
                for k, val in pairs(getgenv().Theme[v]) do
                    library.theme[k] = val
                end
            end)
            if okT then
                WA_Notify("Theme " .. v .. " da ap dung cho element moi. Re-exec de doi toan bo UI.")
            end
        end
    end, 3)

    Set.createButton("Save Config", IC.gear, function()
        local ok, err = WA.SaveConfig()
        if ok then WA_Notify("Da luu config -> " .. "wazure_config.json")
        else WA_Notify("Luu that bai: " .. tostring(err)) end
    end)

    Set.createButton("Load Config", IC.gear, function()
        local ok, err = WA.LoadConfig()
        if ok then
            WA_Notify("Da load config. Mo lai UI de thay gia tri moi.")
        else
            WA_Notify("Load that bai: " .. tostring(err))
        end
    end)

    Set.createButton("Destroy UI", IC.gear, function()
        pcall(function()
            shared._wazure = nil
            if shared._wazure == nil and ScreenGui then ScreenGui:Destroy() end
        end)
    end)

    -- Phan additional tab (demonstrate createAdditionalTab dang hoat dong)
    local Extra = Main.createAdditionalTab()
    Extra.createTabHeader("Extra", "Quick actions")
    Extra.createButton("Reset Character", IC.gear, function()
        pcall(function() plr.Character.Humanoid.Health = 0 end)
    end)
    Extra.createButton("Print Settings", IC.chart, function()
        local ok, enc = pcall(function()
            return game:GetService("HttpService"):JSONEncode(_G.Settings)
        end)
        if ok then print("[W-Azure] Settings: " .. tostring(enc)) end
    end)

    end -- /if Window

    -- UI toggle hotkey (mac dinh RightControl)
    task.spawn(function()
        UIS.InputBegan:Connect(function(input, gp)
            if gp then return end
            local k = getgenv().WA_UIKey or Enum.KeyCode.RightControl
            if input.KeyCode == k then
                pcall(function()
                    if shared._wazure then
                        shared._wazure.Enabled = not shared._wazure.Enabled
                    end
                end)
            end
        end)
    ...
    WA_Notify("W-Azure Restored loaded. Menu da san sang.", 4)
end


-- [W-Azure Restored] _G.Settings da duoc khoi tao + merge defaults o phan MENU phia tren.
-- Giu lai fallback neu user chi chay engine ma khong chay menu.
if type(_G.Settings) ~= "table" then
    _G.Settings = {}
end
do
    local __defaults = {
        ["Selected_Weapons"]="Melee",["Auto_Farm_Level"]=false,["Auto_Legendary_Sword"]=false,
        ["Auto_Legendary_Sword_Hop"]=false,["Auto_Farm_Saber"]=false,["Auto_New_World"]=true,
        ["Auto_Third_Sea"]=true,["Grab_All_Fruits"]=false,["Grab_All_Fruits_Hop"]=false,
        ["Grab_All_Fruits_Hop_Level"]=1500,["Auto_Store_All_Fruits"]=false,["Auto_Eat_Fruits"]=false,
    ...
    for __k, __v in pairs(__defaults) do
        if _G.Settings[__k] == nil then _G.Settings[__k] = __v end
    end
end
...
local RepStorage=c; -- [W-Azure Restored] bien nay bi thieu trong ban 2023 (Auto Legendary Sword chay khong co tac dung)
...
-- [W-Azure Restored] getupvalues khong ton tai tren nhieu executor -> ban cu crash o day
local Y = nil
do
    local gu = getupvalues or debug and debug.getupvalue and function(f)
        local out, i = {}, 1
        while true do
    ...
    local ok, res = pcall(function()
        if gu then
            local t = gu(Q)
            if type(t) == "table" then return t[2] end
        end
    end)
    if ok then Y = res end
end
if Y == nil then
    warn("[W-Azure] Khong lay duoc activeController (getupvalues). Fast Attack se bi gioi han.")
end
...
getgenv().hit();
Usefastattack=(getgenv().WA_FastAttack~=false); -- [W-Azure Restored] dieu khien tu menu
...
task.spawn(function() 
while true do wait();
if Usefastattack and (getgenv().WA_FastAttack~=false) and _G.Settings.Auto_Farm_Level then
 coroutine.wrap(function() 
getgenv().hit();
end)();
end;
...
-- [W-Azure Restored] loadstring remote khong duoc bao ve -> neu URL chet thi toan bo script dung.
pcall(function()
    local okLoad, resLoad = pcall(function()
        return game:HttpGet("https://androssy.net/TrashXenon/Script6.lua")
    end)
    if okLoad and type(resLoad) == "string" and #resLoad > 0 then
        local fn = loadstring(resLoad)
        if fn then fn() end
    else
        warn("[W-Azure] Bo qua Script6.lua (khong tai duoc).")
    end
end);
...
-- ===========================================================================
--  INFINITE YIELD (phan duoc ghep vao source W-Azure)
--  [W-Azure Restored] boc trong pcall + gate bang getgenv().WA_LOAD_IY
--  (mac dinh van load; dat getgenv().WA_LOAD_IY = false truoc khi exec de tat)
-- ===========================================================================
if getgenv().WA_LOAD_IY ~= false then
local __wa_iy_ok, __wa_iy_err = pcall(function()
...
if not __wa_iy_ok then
    warn("[W-Azure] Infinite Yield loi (khong anh huong menu/farm): " .. tostring(__wa_iy_err))
end
end
