        ...
        if ((typeof(gc) == "function") and (getfenv(gc).script == plr.Character.Soru)) then
          for _, v in next, getupvalues(gc) do
            if (typeof(v) == "table") then
              repeat wait(Sec) v.LastUse = 0 until not Var or not (plr.Character and plr.Character:FindFirstChild("Humanoid")) or (plr.Character.Humanoid.Health <= 0) -- [FIX PERF 30] nil-guard
            end
          end
        end
      end
...
spawn(function()
    ...
    while __DIO_wait(5) do -- [FIX PERF 64] 1s x 3 remote -> 5s + break som (tim thay thi khong goi con lai): 3/s -> 0.2/s
...
spawn(function()
    local __prevElite = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
    while __DIO_wait(2) do -- [FIX FPS 2] 0.1s -> 2s: cu goi remote EliteHunter 10 lan/s ca khi treo
        pcall(function()    
            local __v = "Elite Progress : " .. replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress")
...
spawn(function()
    local __prevCake = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
    while __DIO_wait(2) do -- [FIX FPS 2] 0.2s -> 2s: remote CakePrinceSpawner 5 lan/s -> 0.5 lan/s
        pcall(function()
            ...
            pcall(function()
                ...
                if boss or portalOpen then  
                    if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 and boss.PrimaryPart then  
                        -- [FIX CAKE FPS] Kill2 da tu xu ly TP + BringEnemy.
                        -- Bo 2 _tp trung lap truoc Kill2 -> tranh tao/cancel tween lien tuc.
                ...
                -- [FIX CAKE FPS] GhostClean da co worker cleanup 5s; khong quet 2 bang moi iteration.
                -- pcall(__DIO_GhostClean)
                ...
                if bestMob then
                    -- [FIX CAKE LAG] __DIO_wait(Sec) = 4/s thay task.wait() 60/s + ghost skip
                    repeat
                        __DIO_wait(Sec)
                        if not bestMob.Parent or bestMob.Humanoid.Health <= 0 then break end
                        -- [FIX CAKE FPS] Dung object da cache o dau vong; tranh pcall + path lookup 4 lan/s.
                        local portalNow = other and other.Transparency == 0
                        if portalNow or enemies:FindFirstChild("Cake Prince") then break end

                        -- [FIX CAKE FPS] Kill2 da tu TP den vi tri farm + BringEnemy.
                        -- _tp truoc do la tween trung lap, tao task.spawn moi lan goi.
                        Attack.Kill2(bestMob, true)
                        -- Ghost check: 3s ban skill / 6s chon mob khac
                        if __DIO_GhostCheck(bestMob) then
                            -- Tim mob ke tiep thay vi tiep tuc ghost mob nay
...
spawn(function()
    ...
    while __DIO_wait(1.5) do -- [FIX FPS 2] 0.2s -> 1.5s: giam remote InfoLeviathan khi treo
        pcall(function()
