...
do
  ...
  Brazier2 = false
  Brazier3 = false
  Sec = 0.25 -- [FIX PERF 14] 0.1 -> 0.25 (doi chieu Bear Hub: da phan loop cua ho 1-5s, chi 12 loop 0.1s): 96 loop dung Sec giam 2.5x churn; farm re-target 4 lan/s la du (Bear Hub re-target tung frame van smooth)
  ClickState = 0
  Num_self = 25
end

-- [FIX BOOT-RAMP 1/2] PHAN TICH (suy luan cua user + census 312 loop): hub load xong thi hang tram spawn-loop
-- duoc tao trong CUNG mot luot, va SAVE SETTING bat lai nhieu toggle NGAY LUC load -> tic dau cua cac loop
-- nang XUYEN NHAU dung luc (FPS te ngay khi UI vua xong - dung nhu user suy luan). Sau do cac loop dung
-- Sec/chu ky giong nhau -> chung THUC DUNG FRAME dinh ky = FPS gao nhip dai mai. Giay do bang 2 lop:
--   (a) RAMP: trong 10s dau sau load, tic dau cua moi loop moi duon ra 0.08s/loop (cap 4s) -> nen dan khoi dong.
--   (b) PHASE JITTER vinh duen: moi loop (moi thread) co phase rieng 0..0.15s -> khong bao gio thuc dung frame nua.
-- task.wait() per-frame (85 cho, giu nguyen) KHONG qua day -> vong keo block van giu per-frame that.
__DIO_BootT0 = tick()
__DIO_BootN = 0
__DIO_ThreadPhase = setmetatable({}, {__mode = "k"}) -- weak key: thread chet thi phase tu giai phong
__DIO_wait = function(t)
    ...
    if __co then
        ...
        if __ph == nil then
            -- [FIX BOOT-FIX 1/2] BUG LAN 20: ramp (den +4s) bi NAT VAO PHASE VINH VIEN -> moi loop
            -- tao trong 10s dau cho THEM 0..4.15s vao MOI nhip mai mai = "danh xong mob phai cho may s
            -- moi tween tiep" + fast attack te. Sua: jitter = TI LE voi t (loop 0.03s khong bi deo),
            -- ramp thanh DELAY DOT DAU TIEN rieng (khong luu vao phase).
            ...
            if tick() - __DIO_BootT0 < 10 and __DIO_BootN < 200 then
                ...
                task.wait(math.min(__DIO_BootN * 0.08, 4)) -- ramp CHI tac dung 1 lan (tic dau tien cua loop)
            end
        end
    end
    return task.wait(t + __ph)
end
spawn = function(f) -- [FIX BOOT-RAMP 2/2] ramp tic dau cua loop; het cua so boot thi spawn nhu thuong (nut bam UI khong bi treo vi di lam sau load)
    if tick() - __DIO_BootT0 < 10 and __DIO_BootN < 200 then
        __DIO_BootN = __DIO_BootN + 1
...
-- ==============================================================
-- [SAVE SETTING] - GHÉP TỪ FILE BANANA: tự lưu các Setting vừa bật,
-- sang server khác (hop) script vẫn tự bật lại các chức năng đó.
-- Gồm: SaveSettings / LoadSettings / GetSetting / SetSetting (y hệt Banana)
-- ==============================================================
...
SaveSettings = function()
    ...
    local ok = pcall(function()
        writefile(SaveFileName, game:GetService("HttpService"):JSONEncode(_G.SaveData))
    end)
    return ok
end
...
EquipWeapon = function(text)
  ...
  -- [FIX PERF 59] da cam dung vu khi thi khong EquipTool lai (truoc day goi 4 lan/s moi lan farm retarget)
  if plr.Character and plr.Character:FindFirstChild(text) then return end
  if plr.Backpack:FindFirstChild(text) then
	if plr.Character and plr.Character:FindFirstChild("Humanoid") then plr.Character.Humanoid:EquipTool(plr.Backpack:FindFirstChild(text)) end -- [FIX PERF 30] nil-guard
  end
end
...
-- [FIX MODEFARM 1/2] Helper vi tri danh quai dung chung cho Attack.Kill/Kill2/KillSea (moi toggle farm di qua day).
-- Mode "Normal": cao do = slider "Farm Distance" (mac dinh 20; Fruit 10 / vu khi khac 30).
-- Mode "Obrit": quay vong tron QUANH MOB ban kinh = FarmDistance, cao 12, mat LUON huong ve mob;
-- goc quay theo tick() (0.8 rad/s ~ 1 vong/8s) nen on dinh bat ke tan so loop goi Kill.
_G.ModeFarm = _G.ModeFarm or "Normal"
_G.FarmDistance = _G.FarmDistance or 20
__DIO_OrbitA = 0
__DIO_FarmPos = function(modelCF, h, zOff, yawDeg)
    if _G.ModeFarm == "Obrit" then
        local r = math.max(tonumber(_G.FarmDistance) or 20, 10)
        __DIO_OrbitA = (tick() * 0.8) % (math.pi * 2)
        local px = modelCF.Position + Vector3.new(math.cos(__DIO_OrbitA) * r, 12, math.sin(__DIO_OrbitA) * r)
        return CFrame.new(px, modelCF.Position)
    end
	...
	Attack.Kill = function(model, Succes)
		if model and Succes then
			...
			if ToolTip == "Blox Fruit" then
				_tp(__DIO_FarmPos(model.HumanoidRootPart.CFrame, 10, 0, 90)) -- [FIX MODEFARM] cao do/obrit theo slider
			else
				_tp(__DIO_FarmPos(model.HumanoidRootPart.CFrame, 30, 0, 180)) -- [FIX MODEFARM]
			end
	...
	Attack.Kill2 = function(model, Succes)
		if model and Succes then
			...
			if ToolTip == "Blox Fruit" then
				...
				_tp(__DIO_FarmPos(model.HumanoidRootPart.CFrame, 30, 8, 180)) -- [FIX MODEFARM]
			end
			if RandomCFrame then
				task.wait(0.1)
				_tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25))
...
local TweenInfoBring = TweenInfo.new(0.45, Enum.EasingStyle.Linear) -- [FIX PERF 15] 0.3 -> 0.45 (thong so Bear Hub): tween dai hon thi it bi goi lai thi giua chung

_G.BringRange = _G.BringRange or 180 -- [USER-EDIT] chinh tay user tu ban 99999: 300 -> 180 (giam VFX/FPS khi farm)
_G.MaxBringMobs = _G.MaxBringMobs or 4 -- [USER-EDIT] chinh tay user: 6 -> 4
...
BringEnemy = function(Mon)
    ...
    -- [FIX PERF 13] doi chieu Bear Hub: ho chi bring ~1 lan/9s (loop duty-cycle 1Hz: bring roi
    -- nghi 8s) van gom du quai vi attr "Tweening" + WalkSpeed=0 giu chan quai da gom. 0.5 -> 2.5s
    -- de tween mob gan nhu khong chay lien tuc (2.4 Create/s thay vi 60 nhu ban goc).
...
-- [FIX INV-SPAM] phan tich: GetIn/GetM/GetWP deu InvokeServer("getInventory") truc tiep,
-- ma chung lai duoc goi trong cac loop 0.1s (Rip Indra, Skull Guitar, Bisento, nut mua...)
-- => spam remote 10-20 lan/s gay lag. FIX: cache ket qua getInventory 2 giay dung chung 3 ham.
...
GetM = function(Name)
  for _,tab in pairs(GetInventoryCached()) do -- [FIX INV-SPAM] dung cache thay invoke truc tiep
...
GetWP = function(nametool)
  for _,v4 in pairs(GetInventoryCached()) do -- [FIX INV-SPAM] dung cache thay invoke truc tiep
...
getInfinity_Ability = function(Method, Var)
  ...
  if Method == "Soru" and Var then
    for _,gc in next, getgc() do
      if plr.Character and plr.Character:FindFirstChild("Soru") then -- [FIX PERF 30] nil-guard (Soru mat khi chet)
        if ((typeof(gc) == "function") and (getfenv(gc).script == plr.Character.Soru)) then
          for _, v in next, getupvalues(gc) do
            if (typeof(v) == "table") then
              repeat task.wait(Sec) v.LastUse = 0 until not Var or not (plr.Character and plr.Character:FindFirstChild("Humanoid")) or (plr.Character.Humanoid.Health <= 0) -- [FIX PERF 30] nil-guard
            end
          end
        end
      end
  ...
  elseif Method == "Energy" and Var then
    -- [FIX PERF 67] moi lan bat toggle tao 1 connection MOI (bat 10 lan = 10 ket noi tinh Energy);
    -- ngat ket noi cu truoc khi tao moi.
...
task.spawn(function()while __DIO_wait(0.1)do if block and block.Parent==workspace then if shouldTween then getgenv().OnFarm=true else getgenv().OnFarm=false end else getgenv().OnFarm=false end end end) -- [FIX SPAM-1] task.wait() 60/s -> __DIO_wait(0.1) 10/s: loop chi check flag, 10/s du
-- [FIX BUG-2 CRITICAL] Ban cu: task.wait() = 60/s. Khi OnFarm=true: GetChildren + n*CanCollide=false
-- MOI FRAME du part da la false (1,500 redundant physics writes/s). Khi OnFarm=false van viet =true.
-- Fix: (a) Them guard "and e.CanCollide" / "and not e.CanCollide" -> chi write khi gia tri THAY DOI;
--      (b) Phan CFrame teleport giu nguyen task.wait() (per-frame can thiet de theo sat mob);
--      (c) Phan CanCollide tach rieng vao gate 0.1s (10/s du, khong can 60/s).
local __DIO_BlockNoClipLast = 0
task.spawn(function()local a=game.Players.LocalPlayer;repeat task.wait()until a.Character and a.Character.PrimaryPart;block.CFrame=a.Character.PrimaryPart.CFrame;while task.wait()do pcall(function()if getgenv().OnFarm then if block and block.Parent==workspace then local b=a.Character and a.Character.PrimaryPart;if b and(b.Position-block.Position).Magnitude<=200 then b.CFrame=block.CFrame else block.CFrame=b.CFrame end end;local c=a.Character;if c and tick()-__DIO_BlockNoClipLast>=0.1 then __DIO_BlockNoClipLast=tick();for d,e in pairs(c:GetChildren())do if e:IsA("BasePart")and e.CanCollide then e.CanCollide=false end end end else local c=a.Character;if c and tick()-__DIO_BlockNoClipLast>=0.1 then __DIO_BlockNoClipLast=tick();for d,e in pairs(c:GetChildren())do if e:IsA("BasePart")and not e.CanCollide then e.CanCollide=true end end end end end)end end)
...
function checkinventory(v)
    -- [FIX PERF 56] PHAN TICH (hot path tween+farm): ham nay INVOKE "getInventory" TRUC TIEP
    -- moi lan goi; duoc goi trong requestentrance (chay moi lan _tp = 4 lan/s khi farm) ->
    -- o SEA3 la 4 remote getInventory moi giay + parse toan bang inventory lien tuc DANG TWEEN
    -- -> FPS tut rat nhanh. GetInventoryCached() co san (cache 2s) thay vao, ket qua dung nhu cu.
    if v then
...
-- [FIX PERF 57] InArea ban dau quet TOAN BO WorldOrigin.Locations moi lan goi; ket qua chi
-- phu thuoc vi tri -> cache theo o luoi 25 stud, TTL 0.5s (chi dung cho duong nong _tp).
...
function CanBypassTeleport(x)
    -- [FIX PERF 57] PHAN TICH (hot path tween+farm): ban cu goi InArea(x) TRUOC = quet 50-100
    -- Locations (FindFirstChild Mesh tung vung) MOI LAN _tp (4 lan/s khi farm), trong khi dieu
    -- kien re nhat la khoang cach <= 3500 (farm cung bai chi chay nhanh nay va return false NGAY).
    -- Doi thu tu khong doi ket qua boolean (false neu bat ky dieu kien nao sai).
    if GetDistance(x.Position) <= 3500 then
        return false
    end
    local AreaName = __DIO_InAreaCached(x).Name
...
function BypassTP(Target, __SkipCanCheck)
    -- [FIX PERF 58] PHAN TICH: ban cu kiem tra CanBypassTeleport (InArea quet Locations + GetDistance)
    -- LAN THU HAI va goi GetBypassCFrame DU CACH o day. Caller _tp vua kiem CanBypassTeleport roi
    -- -> truyen __SkipCanCheck = true de bo qua; GetBypassCFrame chi goi dung 1 lan.
    local Character = LocalPlayer.Character
    ...
    local TargetTP = nil
    if __SkipCanCheck or CanBypassTeleport(Target) then
        TargetTP = GetBypassCFrame(Target)
    end
    if TargetTP and TargetTP:FindFirstChild("Part") then -- [FIX PERF 58]
...
-- [FIX MISSING-FN] 3 ham duoc goi o nhieu cho (topos: Race/Prehistoric/TpLab; StopTween: TpLab;
-- AutoHaki: Kill Player Quest) nhung KHONG DUNG DINH NGHIA -> loi nil runtime (pcall nuot nen
-- compile van 0 va tinh nang chet cam). Dinh nghia theo dung y nghia chuan cua hub.
...
AutoHaki = function()
    -- tu bat Buso khi chua bat (game them "HasBuso" vao character khi dang bat)
...
spawn(function()
  while __DIO_wait(0.1) do -- [FIX LAG] mỗi frame -> 0.1s
    pcall(function()
      if _G.SailBoat_Hydra or _G.WardenBoss or _G.AutoFactory or _G.HighestMirage or _G.HCM or _G.PGB or _G.Leviathan1 or _G.UPGDrago or _G.Complete_Trials or _G.TpDrago_Prehis or _G.BuyDrago or _G.AutoFireFlowers or _G.DT_Uzoth or _G.AutoBerry or _G.Prefully or _G.Prehis_Find or _G.Prehis_Skills or _G.Prehis_DB or _G.Prehis_DE or _G.FarmBlazeEM or _G.Dojoo or _G.CollectPresent or _G.AutoLawKak or _G.TpLab or _G.AutoPhoenixF or _G.AutoFarmChest or _G.AutoHytHallow or _G.LongsWord or _G.BlackSpikey or _G.AutoHolyTorch or _G.TrainDrago  or _G.AutoSaber or _G.FarmMastery_Dev or _G.CitizenQuest or _G.AutoEctoplasm or _G.KeysRen or _G.Auto_Rainbow_Haki or _G.obsFarm or _G.AutoBigmom or _G.Doughv2 or _G.AuraBoss or _G.Raiding or _G.Auto_Cavender or _G.TpPly or _G.Bartilo_Quest or _G.Level or _G.FarmEliteHunt or _G.AutoZou or _G.AutoFarm_Bone or getgenv().AutoMaterial or _G.CraftVM or _G.FrozenTP or _G.ShotHL or _G.TPDoor or _G.AcientOne or _G.AutoFarmNear or _G.AutoRaidCastle or _G.DarkBladev3 or _G.AutoFarmRaid or _G.Auto_Cake_Prince or _G.Addealer or _G.TPNpc or _G.TwinHook or _G.FindMirage or _G.FrozenDimension or _G.FarmChestM or _G.Shark or _G.TerrorShark or _G.Piranha or _G.MobCrew or _G.SeaBeast1 or _G.FishBoat or _G.AutoPole or _G.AutoPoleV2 or _G.Auto_SuperHuman or _G.AutoDeathStep or _G.Auto_SharkMan_Karate or _G.Auto_Electric_Claw or _G.AutoDragonTalon or _G.Auto_Def_DarkCoat or _G.Auto_God_Human or _G.Auto_Tushita or _G.AutoMatSoul or _G.AutoKenVTWO or _G.AutoSerpentBow or _G.AutoFMon or _G.Auto_Soul_Guitar or _G.TPGEAR or _G.AutoSaw or _G.AutoTridentW2 or _G.AutoEvoRace or _G.AutoGetQuestBounty or _G.MarinesCoat or _G.TravelDres or _G.Defeating or _G.DummyMan or _G.Auto_Yama or _G.Auto_SwanGG or _G.SwanCoat or _G.AutoEcBoss or _G.Auto_Mink or _G.Auto_Human or _G.Auto_Skypiea or _G.Auto_Fish or _G.CDK_TS or _G.CDK_YM or _G.CDK or _G.AutoFarmGodChalice or _G.AutoFistDarkness or _G.AutoMiror or _G.Teleport or _G.AutoKilo or _G.AutoGetUsoap or _G.Praying or _G.TryLucky or _G.AutoColShad or _G.AutoUnHaki or _G.Auto_DonAcces or _G.AutoRipIngay or _G.DragoV3 or _G.DragoV1 or _G.SailBoats or NextIs or _G.FarmGodChalice or _G.IceBossRen or senth or senth2 or _G.Lvthan or _G.beasthunter or _G.DangerLV or _G.Relic123 or _G.tweenKitsune or _G.Collect_Ember or _G.AutofindKitIs or _G.snaguine or _G.TwFruits or _G.tweenKitShrine or _G.Tp_LgS or _G.Tp_MasterA or _G.tweenShrine or _G.FarmMastery_G or _G.FarmMastery_S or _G.FarmBoss or _G.AutoFarmAllBoss or _G.AutoFishSlap or _G.FarmTyrant or _G.FarmPhaBinh or _G.AutoSpawnCP or _G.AutoBerryH or _G.AutoChestBP or _G.FarmEliteHop or _G.AutoHop_Dough or _G.AutoDoughKing or _G.AutoAttackDoughKing or _G.AutoChipFruit or _G.AutoChipBeli or _G.StartEvent or _G.AutoMysticIsland or _G.AutoPlayerHunter or _G.SafeMode or _G.AutoKillMob or _G.AutoStartPrehistoric or _G.AutoUnHaki or _G.AutoFarmIsland or _G.AutoFarmDungeon or _G.AutoFarmCandy or _G.AutoTP_Gift or _G.AutoTPGift or _G.AutoTPAndCollect or _G.MasterAutoLevel or _G.MasterAutoCandy or _G.TPFloor1 or _G.TPFloor2 or _G.TPFloor3 or _G.TPFloor4 or _G.FarmEliteH or _G.Safemode or _G.Greybeard or _G.ResetPH then -- [FIX FLYBUSY-FLAG] 4 toggle co loop rieng nhung khong nam trong cond list -> stomper stomp shouldTween moi 0.1s -> khong bao gio bay (cung benh BuyMelee cu)          
       shouldTween = true
        ...
        if tick() - (__DIO_LastNoclipDecor or 0) < 0.25 then return end -- [FIX PERF 68] phan Highlight + GetDescendants/CanCollide nang nhat va chay SUOT khi farm: 10Hz -> 4Hz du muot (shouldTween + BodyClip van giu 10Hz)
    ...
    Test.Parent = plr.Character
        ...
        -- [FIX BUG-5 MEDIUM] Guard CanCollide: chi write khi gia tri THAY DOI (tranh 800 redundant physics write/s)
        for _, no in pairs(plr.Character:GetDescendants()) do if no:IsA("BasePart") and no.CanCollide then no.CanCollide = false end end
      elseif not __DIO_FlyBusy then -- [FIX FLYBUSY 1/3] vong nay stomp shouldTween=false moi 0.1s khi khong co toggle farm -> tween cua BuyMelee bi Cancel ngay = nhan vat dung yen ("chi hien notify"). Khi wrapper dang bay thi KHONG stomp.
        shouldTween = false
...
local redzlib = loadstring(game:HttpGet("https://raw.githubusercontent.com/thidinh240581-maker/Script-/refs/heads/main/Library-Redz-Pulic-Source"))()
...
local Tabs = {
    Info = Window:MakeTab({ Title = "Info Discord", Icon = "Info" }), 
    Status = Window:MakeTab({ Title = "Status Server", Icon = "crown" }),
...
-- ==============================================================
-- [SAVE SETTING] - GHÉP TỪ FILE BANANA: bọc AddToggle/AddDropdown/AddSlider
-- của TẤT CẢ tab -> control nào cũng tự lưu + tự khôi phục sau hop.
-- Key lưu: "UI/<Tên Tab>/<Tên control>" -> ổn định giữa các lần chạy.
-- ==============================================================
do
        ...
        if type(_AddToggle) == "function" then
            function tab:AddToggle(Setting)
                ...
                if Setting and saved ~= nil then
                    Setting.Default = saved -- nạp lại trạng thái lần trước
                end
                ...
                -- đảm bảo cờ _G được áp dụng kể cả khi library không tự gọi Callback(Default)
...
-- [CACHE] Raw string từ remote được lưu global để Farm paragraph đọc lại
-- → loại bỏ InvokeServer thứ 2 (1.5 calls/s → 1 call/s)
__DIO_CakePrinceRaw = ""

spawn(function()
    local __prevCP = nil
    while __DIO_wait(3) do -- [FIX SPAM-3] 1s -> 3s: status khong can cap nhat nhanh (20 remote/phut -> 20/3)
        pcall(function()
            local cakePrince = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")
            if type(cakePrince) ~= "string" then return end
            __DIO_CakePrinceRaw = cakePrince   -- cập nhật cache cho Farm loop
...
spawn(function()
    ...
    while __DIO_wait(10) do -- [FIX SPAM-4] 5s -> 10s: thanh kiem huyen thoai spawn rat hiem, 10s la du
        local swordStatus = "Not Found"
        local d1 = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer", "1")
        if d1 then
            swordStatus = "Shisui ✅"
        else
            local d2 = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer", "2")
            if d2 then
                swordStatus = "Wando ✅"
            else
                local d3 = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer", "3")
                if d3 then
                    swordStatus = "Saddi ✅"
                end
            end
        end
        if swordStatus ~= __prevSword then __prevSword = swordStatus LegendarySwordCheck:SetDesc(swordStatus) end -- chi SetDesc khi doi
    end
end)

local BoneCount = Tabs.Status:AddParagraph("Bone", "")

spawn(function()
    -- [FIX BUG-6 MEDIUM] Ban cu: InvokeServer + SetDesc MO 1s du so luong bone khong doi.
    -- Fix: (a) 1s -> 5s (so xuong khong can cap nhat nhanh); (b) prev check chi SetDesc khi doi;
    --      (c) pcall bao ve khoi nil error khi InvokeServer tra ve nil.
    local __prevBone = nil
    while __DIO_wait(8) do -- [FIX SPAM-5] 5s -> 8s: so luong bone it khi thay doi lien tuc
        pcall(function()
            local bones = game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Bones", "Check")
            local desc = "You Have: " .. tostring(bones) .. " Bones"
            if desc ~= __prevBone then __prevBone = desc BoneCount:SetDesc(desc) end
        end)
    end
end)
...
spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 62] 0.5 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        pcall(function()
...
-- ============================================================
-- [GHOST MOB FIX v2] Persistent skip-table — Bone / Cake / Level
--
-- VẤN ĐỀ CŨ: __DIO_GhostTrack() reset timer mỗi lần outer loop lặp
-- → cùng ghost mob bị attack lại từ 0 mãi mãi (vô dụng).
--
-- CÁCH FIX ĐÚNG:
--   __DIO_GhostMobs[mob]  = bảng {hp, t, skillFired}  per mob instance
--   Khi ghost xác nhận (6s)  → __DIO_GhostSkip[mob] = true
--   Mob selection           → bỏ qua mob có __DIO_GhostSkip[mob]
--   Cleanup tự động         → mob chết/remove → xoá khỏi cả 2 bảng
--   Toggle OFF              → xoá toàn bộ bảng
--
-- Khi phát hiện ghost: farm chờ 0-3s, bắn skill Z/X/C ở 3s, đánh dấu skip ở 6s.
-- Mob tìm tiếp sẽ BỎ QUA mob đã mark → không bao giờ tween về ghost cũ nữa.
-- ============================================================
local __DIO_GhostData = {}  -- [mob] = {hp=N, t=tick(), skill=false}
local __DIO_GhostSkip = {}  -- [mob] = true  → bỏ qua khi chọn mob

-- Dọn dẹp mob đã chết / bị xóa khỏi cả 2 bảng
...
-- Xóa toàn bộ khi tắt toggle
local function __DIO_GhostReset()
    __DIO_GhostData = {}
    __DIO_GhostSkip = {}
end

-- Trả về true = mob này bị skip (đã xác nhận ghost)
local function __DIO_IsGhost(mob)
    return __DIO_GhostSkip[mob] == true
end

-- Gọi SAU mỗi Attack.Kill/Kill2.
-- Trả về true = ghost xác nhận → caller phải break và tìm mob khác.
local function __DIO_GhostCheck(mob)
    ...
    if not d then
        -- Lần đầu gặp mob này → bắt đầu theo dõi
        __DIO_GhostData[mob] = { hp = curHp, t = tick(), skill = false }
        return false
    end

    if curHp ~= d.hp then
        -- HP đổi → mob nhận damage bình thường → reset timer cho mob đó
        d.hp    = curHp
        d.t     = tick()
        d.skill = false
        return false
    ...
    if elapsed >= 6 then
        -- 6s không gây được sát thương → ĐÁNH DẤU SKIP VĨNH VIỄN
        -- Mob này sẽ bị bỏ qua ở mọi vòng chọn mob tiếp theo cho đến khi nó chết
        __DIO_GhostSkip[mob] = true
        __DIO_GhostData[mob] = nil
        return true  -- caller: break, tìm mob khác
    end

    if elapsed >= 3 and not d.skill then
        -- 3s không gây damage → bắn skill để phá ghost
        d.skill = true
        task.spawn(function()  -- spawn riêng để không block farm loop
            pcall(function()
                EquipWeapon(_G.SelectWeapon)
                local eq = plr.Character and plr.Character:FindFirstChildOfClass("Tool")
                local wt = eq and eq.ToolTip or "Melee"
                if wt == "Sword" then
                    Useskills("Sword","Z") task.wait(0.15)
                    Useskills("Sword","X")
                elseif wt == "Blox Fruit" then
                    Useskills("Blox Fruit","Z") task.wait(0.15)
                    Useskills("Blox Fruit","X") task.wait(0.15)
                    Useskills("Blox Fruit","C")
                elseif wt == "Gun" then
                    Useskills("Gun","Z") task.wait(0.15)
                    Useskills("Gun","X")
                else
                    Useskills("Melee","Z") task.wait(0.15)
                    Useskills("Melee","X") task.wait(0.15)
                    Useskills("Melee","C")
                end
            end)
        end)
    ...
    return false  -- chưa xác nhận ghost, tiếp tục attack
end

-- Cleanup định kỳ 5s để không tích lũy dead refs
task.spawn(function()
    while true do
        task.wait(5)
...
-- ============================================================
-- [FIX FARM LEVEL] Khai báo trước toggle để callback capture
-- đúng upvalue (bug cũ: local đặt SAU toggle → callback set global
-- khác với biến local mà spawn loop dùng → tắt toggle không reset được)
-- ============================================================
...
FarmLevel = Tabs.Main:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            teleporting = false
            pcall(__DIO_GhostReset)  -- xóa skip list khi tắt
        end
    end
})

-- [alreadyTeleported + teleporting đã được khai báo TRƯỚC toggle để callback dùng đúng upvalue]
        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- Dọn ghost list đầu mỗi iteration (xóa mob đã chết)
                    pcall(__DIO_GhostClean)

                    local foundMob = false
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and Attack.Alive(v)
                           and not __DIO_IsGhost(v)  -- BỎ QUA mob đã xác nhận ghost
                        then
                            foundMob = true
                            repeat
                                __DIO_wait(Sec)
                                if not v.Parent or not v:FindFirstChild("HumanoidRootPart") then break end
                                ...
                                -- Ghost check: 3s bắn skill / 6s đánh dấu skip vĩnh viễn
                                if __DIO_GhostCheck(v) then
                                    foundMob = false
                                    break
                                end
                            until not _G.Level or not v.Parent or v.Humanoid.Health <= 0
                            if foundMob then break end  -- đã giết xong, thoát for
                        end
                    end
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
      if _G.AutoFarmNear then
        for i,v in pairs(workspace.Enemies:GetChildren()) do
          if v:FindFirstChild("Humanoid") or v:FindFirstChild("HumanoidRootPart") then
            if v.Humanoid.Health > 0 then
              repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoFarmNear) until not _G.AutoFarmNear or not v.Parent or v.Humanoid.Health <= 0
            end
          end
        end
      end
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.3 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
            ...
            if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
              if v.Name then
                if (v.HumanoidRootPart.Position - Root.Position).Magnitude <= 2000 then
                  repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoRaidCastle) until not _G.AutoRaidCastle or not v.Parent or v.Humanoid.Health <= 0 or not workspace.Enemies:FindFirstChild(v.Name)
                end
              end
            end
          end
    ...
    pcall(function()
      if _G.AutoEctoplasm then
		...
		if Attack.Alive(v) then
		  repeat __DIO_wait(Sec) Attack.Kill(v, _G.AutoEctoplasm)until not _G.AutoEctoplasm or not v.Parent or v.Humanoid.Health <= 0		        
	    else
            ...
            local function runChestLoop()
                ...
                task.spawn(function()
                    while _G.AutoChestBP and LocalPlayer.Character and LocalPlayer.Character.Parent do
                        ...
                        __DIO_wait(0.3) -- [FIX PERF 47] 0.1 -> 0.3: set thuang RootPart.CFrame 10 lan/s thanh ~3 lan/s du de bat chest
                    end
                    IsFarming = false
                end)
            end
...
task.spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 62] 0.35 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
...
-- [FIX SPAM-2] EliteHunter Progress: 1 remote duy nhat 3s, luu cache cho ca 2 paragraph
__DIO_EliteProgressCache = "0" -- chia se giua Process paragraph va EliteHunter Spawn paragraph
local Process = Tabs.Main:AddParagraph("Elites Process", "")
spawn(function()
    local __prevElite = nil
    while __DIO_wait(3) do -- [FIX SPAM-2] 2s -> 3s + cap nhat cache cho loop thu 2 doc lai
        pcall(function()
            local p = replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress")
            __DIO_EliteProgressCache = tostring(p or "0") -- cap nhat cache
            local __v = "Elite Progress : " .. __DIO_EliteProgressCache
...
spawn(function()
    ...
    while __DIO_wait(3) do -- [FIX SPAM-7] 1s -> 3s: spawn status khong can check nhanh
        ...
        -- [FIX SPAM-2] doc tu cache; khong goi InvokeServer lan 2 (loai bo 60 remote/phut)
        local progress = __DIO_EliteProgressCache or "0"
  ...
  while __DIO_wait(Sec) do
    pcall(function()
      if _G.AutoRipIngay then
        -- [FIX RIP INDRA] TIM CA 2 TEN: "rip_indra" (boss Second Sea - Indra Island) va
        -- "rip_indra True Form" (raid boss Lv.5000 spawn tai cung dien Castle on the Sea,
        -- summon bang God's Chalice, drop Dark Dagger 5% + Valkyrie Helm). Ban cu chi tim 1 ten
        -- nen khi boss True Form spawn thi toggle nhin nhu CHET (bo lo quai that trong Enemies).
        local v = GetConnectionEnemies("rip_indra") or GetConnectionEnemies("rip_indra True Form")
        -- [FIX RIP INDRA] fix thu tu toan tu: ban cu "A or B and v" duoc Lua tinh la "A or (B and v)"
        -- -> chua co Dark Dagger ma chua co boss la Attack.Kill(nil) rong roi suot, khong bao gio
        -- ve Castle cho spawn -> toggle dung yen. Gio: CO boss + thieu 1 trong 2 do drop
        -- (Dark Dagger hoac Valkyrie) -> danh; KHONG co boss (hoac du do) -> ve Castle cho spawn.
...
spawn(function()
    local __prevCake = nil
    while __DIO_wait(2) do
        pcall(function()
            -- [FIX REMOTE-DEDUP] Dùng cache từ Status loop thay vì gọi InvokeServer thứ 2
            -- (trước: 1.5 remote/s cho cùng 1 endpoint → nay: 0)
            local Killed = string.match(__DIO_CakePrinceRaw or "", "%d+")
            if Killed then
                local __v = "Killed : " .. (500 - tonumber(Killed) or 0)
                if __v ~= __prevCake then __prevCake = __v MobKilled:SetDesc(__v) end
            end
        end)
    end
end)

-- [FIX MISSING-FN] Auto Farm Cake Prince goi TeleportConditional + TELEPORT_DISTANCE_THRESHOLD nhung CA TEN BI THIEU
-- PHAN TICH LOI: called-but-not-defined (cung class voi topos/StopTween) - port tu Bear Hub lam roi 2 ten nay;
-- compile van 0 vi loi chi no khi toggle bat, pcall nuot -> farm di bo/tp sai ma khong bao loi. Bear Hub L3268-3281:
-- chi _tp khi khoang cach > nguong 15 -> tranh tp lap lai lien tuc khi da dung gan dich.
...
Cake = Tabs.Main:AddToggle({
    ...
    if not Value then pcall(__DIO_GhostReset) end  -- xóa skip list khi tắt
end
})
            ...
            pcall(function()
                ...
                if boss or portalOpen then
                    if boss and boss:FindFirstChild("Humanoid") and boss.Humanoid.Health > 0 and boss.PrimaryPart then
                        if not boss:GetAttribute("Locked") then boss:SetAttribute("Locked", boss.HumanoidRootPart.CFrame) end
                        PosMon = boss:GetAttribute("Locked").Position
                        EquipWeapon(_G.SelectWeapon)
                        BringEnemy(boss)
                        local bEq = plr.Character and plr.Character:FindFirstChildOfClass("Tool")
                        local bFruit = bEq and bEq.ToolTip == "Blox Fruit"
                        local bCF = __DIO_FarmPos(boss.HumanoidRootPart.CFrame, bFruit and 10 or 30, bFruit and 0 or 8, bFruit and 90 or 180)
                        if (block.Position - bCF.Position).Magnitude > 10 then _tp(bCF) end
                        return
                    end  
                ...
                -- [FIX CAKE FPS] Khong tao table moi moi iteration; lookup O(1).
                local CAKE_MOBS = {
                    ["Cookie Crafter"] = true,
                    ["Cake Guard"] = true,
                    ["Baking Staff"] = true,
                ...
                for _, mob in ipairs(enemies:GetChildren()) do
                    if CAKE_MOBS[mob.Name] then
                        local hum = mob:FindFirstChild("Humanoid")
                        local root = mob:FindFirstChild("HumanoidRootPart")
                        if hum and root and hum.Health > 0 and not __DIO_IsGhost(mob) then
                            local dist = (root.Position - hrp.Position).Magnitude
                ...
                if bestMob then
                    -- ============================================================
                    -- [FIX CAKE LAG - ROOT CAUSE]
                    -- Bản cũ: Attack.Kill2(bestMob) gọi MỖI __DIO_wait(Sec) = 4/s
                    --   → _tp() → TweenService:Create + Play mỗi 0.25s = 4 tween/s
                    --   → + BringEnemy (gated 2.5s ok) + EquipWeapon (cheap)
                    --   TweenService:Create trên rendering thread = nguồn lag #1 Cake
                    --
                    -- Fix: TP MỘT LẦN khi tìm thấy mob. Vòng lặp chỉ re-TP khi
                    -- block đã lệch > RE_TP_THRESHOLD stud (mob di chuyển xa).
                    -- BringEnemy vẫn gọi mỗi Sec nhưng nó tự gate 2.5s bên trong.
                    -- FastAttack module (Heartbeat) lo việc đánh — không cần Kill2.
                    -- ============================================================
                    local RE_TP_THRESHOLD = 10  -- stud, chỉ tạo tween khi lệch quá ngưỡng này
                    local isFruit = (function()
                        local eq = plr.Character and plr.Character:FindFirstChildOfClass("Tool")
                        return eq and eq.ToolTip == "Blox Fruit"
                    end)()
                    local fH  = isFruit and 10  or 30
                    local fZ  = isFruit and 0   or 8
                    local fYaw = isFruit and 90 or 180

                    -- Lock mob + set PosMon (BringEnemy cần)
                    if not bestMob:GetAttribute("Locked") then
                        bestMob:SetAttribute("Locked", bestMob.HumanoidRootPart.CFrame)
                    end
                    PosMon = bestMob:GetAttribute("Locked").Position

                    EquipWeapon(_G.SelectWeapon)
                    BringEnemy(bestMob)   -- gom quái lần đầu (gate nội bộ)
                    _tp(__DIO_FarmPos(bestMob.HumanoidRootPart.CFrame, fH, fZ, fYaw))

                    repeat
                        __DIO_wait(Sec)
                        if not bestMob.Parent or bestMob.Humanoid.Health <= 0 then break end

                        local portalNow = other and other.Transparency == 0
                        if portalNow or enemies:FindFirstChild("Cake Prince") then break end

                        BringEnemy(bestMob)   -- gate 2.5s bên trong, gọi nhiều lần không tốn

                        -- Chỉ tạo tween khi block thực sự lệch xa target
                        local tgtCF = __DIO_FarmPos(bestMob.HumanoidRootPart.CFrame, fH, fZ, fYaw)
                        if (block.Position - tgtCF.Position).Magnitude > RE_TP_THRESHOLD then
                            _tp(tgtCF)
                        end

                        if __DIO_GhostCheck(bestMob) then
...
Default = false,
Callback = function(Value)
  _G.AcceptQuest = Value   -- [FIX VAR-MISMATCH] loop dùng AcceptQuest, không phải AcceptQuestC
end
})
            ...
            pcall(function()
                ...
                elseif workspace.Map.CakeLoaf:FindFirstChild("RedDoor") then
                    if GetBP("Red Key") then
                        ...
                        until not getgenv().AutoDoughKing or (plr.Character.HumanoidRootPart.Position - CFrame.new(-2681.97998, 64.3921585, -12853.7363,0.149007782, -1.87902192e-08, 0.98883605,3.60619588e-08, 1, 1.35681812e-08,-0.98883605, 3.36376011e-08, 0.149007782).Position).Magnitude <= 5 -- [FIX PERF 22] CFrame-CFrame = runtime error -> step chot khong bao gio chay; doi thanh khoang cach.Magnitude <= 5
                        EquipWeapon("Red Key")
                    end
...
spawn(function()
    local __prevBone = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
    while __DIO_wait(1) do -- [FIX FPS 2] 0.2s -> 1s (GetM da co cache 2s nen khong con spam remote)
        pcall(function()
            local __v = "Bones : " .. GetM("Bones")
            if __v ~= __prevBone then __prevBone = __v CheckingBone:SetDesc(__v) end
        end)
    end
end)

-- [FARM BONE - GHÉP LOGIC BANANA] đã rà: khối farm dưới đây GIỮ LOGIC BANANA
-- (GetClosestMob TÌM quái gần nhất -> đánh XONG con đó mới chuyển, chỉ khi VỠ HẾT quái
-- mới bay về điểm chờ spawn - KHÔNG bay trước như DIO cũ) + AntiFall giữ người lơ lửng.
Tabs.Main:AddToggle({
    Name = "Auto Farm Bone",
    Description = "",
    Default = false,
    ...
    while __DIO_wait(1) do -- [FIX PERF 62] 0.1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        if _G.AutoFarm_Bone then 
            pcall(function()
                ...
                -- [FIX] THÊM ANTI-FALL GIỮ NHÂN VẬT LƠ LỬNG TRÊN KHÔNG
                ...
                -- [PASSO 2] SE JÁ TEM QUEST OU NÃO QUER ACEITAR, VAI FARMAL
                    ...
                    -- Dọn ghost list đầu mỗi iteration
                    pcall(__DIO_GhostClean)
                    ...
                    local function GetClosestMob()
                        ...
                        for _, mobName in pairs(MOBS) do
                            for _, mob in pairs(workspace.Enemies:GetChildren()) do
                                if mob.Name == mobName and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0
                                   and not __DIO_IsGhost(mob)  -- BỎ QUA ghost mob đã xác nhận
                                then
                    ...
                    if mob then
                        -- [FIX BONE] Ban cu: attack 1 lan roi thoat ra outer loop (1/s).
                        -- Fix: inner repeat bam sat den khi mob chet + ghost fix Z/X/C.
                            EquipWeapon(_G.SelectWeapon)
                        _tp(mob.PrimaryPart.CFrame * CFrame.new(0, 5, 0))
                        repeat
                            __DIO_wait(Sec)
                            if not mob.Parent or not mob:FindFirstChild("HumanoidRootPart") then break end
                            _tp(mob.HumanoidRootPart.CFrame * CFrame.new(0, 5, 0))
                            Attack.Kill(mob, _G.AutoFarm_Bone)
                            -- Ghost check: 3s -> Z/X/C, 6s -> chon mob khac
                            if __DIO_GhostCheck(mob) then
                                -- Ghost 6s: tp sang spawn point khac roi thoat tim mob moi
                                _tp(CFrame.new(-9495.68, 453.58, 5977.34))
                                break
                            end
                        until not _G.AutoFarm_Bone or not mob.Parent or mob.Humanoid.Health <= 0
                    else
                        -- Khong co mob: tp vao spawn cho mob respawn
                        _tp(CFrame.new(-9495.68, 453.58, 5977.34))
                    end      
                end
            end)  
  ...
  while __DIO_wait(Sec) do
    if _G.AutoHytHallow then
      pcall(function()
	    ...
	    if v then
          repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoHytHallow) until v.Humanoid.Health <= 0 or _G.AutoHytHallow == false -- [FIX LAG] task.wait()->__DIO_wait(Sec)
        else
          if not GetBP("Hallow Essence") then
            ...
            repeat wait(.1) _tp(CFrame.new(-8932.322265625, 146.83154296875, 6062.55078125))until _G.AutoHytHallow == false or ((plr.Character.HumanoidRootPart.Position - CFrame.new(-8932.322265625, 146.83154296875, 6062.55078125).Position).Magnitude <= 5) -- [FIX PERF 23] CFrame== hang so float khong bao gio khop -> treo repeat; doi thanh khoang cach
  ...
  while __DIO_wait(Sec) do
    pcall(function()
      if _G.Auto_Random_Bone then    
  	    repeat __DIO_wait(0.1) replicated.Remotes.CommF_:InvokeServer("Bones","Buy",1,1) until not _G.Auto_Random_Bone -- [FIX SPAM-8] naked wait -> 0.1s
      end
    end)
  end
end)
...
spawn(function()
  local function processEnemy(v, EnemyName)
    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
      if v.Name == EnemyName then repeat __DIO_wait(Sec) Attack.Kill(v,getgenv().AutoMaterial) until not getgenv().AutoMaterial or not v.Parent or v.Humanoid.Health <= 0 end
    end
  end
  ...
  while __DIO_wait(Sec) do
    pcall(function()
      if _G.FarmMastery_S then
        if SelectIsland == "Cake" then
          for _, v in next, GetInventoryCached() do -- [FIX PERF 63] invoke truc tiep 4 lan/s khi farm kiem mastery -> cache 2s (nen getInventory KHONG NEN goi trong loop)          
              ...
              if v.Type == "Sword" then
                ...
                if tonumber(v.Mastery) >= 1 or tonumber(v.Mastery) <= 599 then
                  ...
                  if GetBP(SwordName) then                    
		            if v then
                      repeat __DIO_wait(Sec) Attack.Sword(v,_G.FarmMastery_S) until _G.FarmMastery_S == false or not v.Parent or v.Humanoid.Health <= 0		                  
		            else
		              _tp(CFrame.new(-1943.676513671875, 251.5095672607422, -12337.880859375)) 
		            end                    
                  else
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] .2 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
...
-- [FIX BUG-1 CRITICAL] Ban cu dung Stepped:Connect = 60 lan/s GetDescendants character
-- (~200 objects * 2 IsA * 60 = 24,000 ops/s) du khi NoClip=false.
-- Fix: (a) Dung __DIO_wait(0.04) thay Stepped -> 25 lan/s (giam 58%),
--      (b) Kiem tra part.CanCollide truoc khi set -> bo redundant physics write,
--      (c) Loop tu thoat khi NoClip=false (khong ton CPU khi tat).
spawn(function()
    while true do
        if getgenv().NoClip then
            ...
            __DIO_wait(0.04) -- 25/s: du muot de noclip, giam 58% so voi Stepped 60/s
        else
            __DIO_wait(0.1) -- idle khi tat: kiem tra 10 lan/s (rat nhe)
        end
    end
end)
          ...
          if replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
            if replicated.Remotes.CommF_:InvokeServer("GetUnlockables").FlamingoAccess ~= nil then
              ...
              if replicated.Remotes.CommF_:InvokeServer("ZQuestProgress", "Check") == 0 then
                ...
                if v then
                  repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoZou) until not _G.AutoZou or not v.Parent or v.Humanoid.Health <= 0
                  Check = 2
              ...
              elseif replicated.Remotes["CommF_"]:InvokeServer("ZQuestProgress", "Check") == 1 then
                ...
                if v then
                  ...
                  repeat wait() _tp(CFrame.new(2288.802, 15.1870775, 863.034607)) until not _G.AutoZou or (Root.Position - CFrame.new(2288.802, 15.1870775, 863.034607).Position).Magnitude <= 5 -- [FIX PERF 60a] so sanh float == khong bao gio khop -> step khong bao gio xong
                  if ((Root.Position - CFrame.new(2288.802, 15.1870775, 863.034607).Position).Magnitude <= 5) then notween(CFrame.new(2288.802, 15.1870775, 863.034607)) end -- [FIX PERF 60a]
                end
              end
            else
            if replicated.Remotes.CommF_:InvokeServer("GetUnlockables").FlamingoAccess == nil then
            ...
            if replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 0 then
              if string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Swan Pirates") and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "50") and plr.PlayerGui.Main.Quest.Visible == true then                
                ...
                if v then
                  pcall(function() repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoZou) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoZou == false or plr.PlayerGui.Main.Quest.Visible == false end)                    
                else
            ...
            elseif replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 1 then
              ...
              if v then
                repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoZou) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoZou == false
              else
            ...
            elseif replicated.Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 2 then
              repeat wait() _tp(CFrame.new(-1836, 11, 1714)) until not _G.AutoZou or (Root.Position - CFrame.new(-1836, 11, 1714).Position).Magnitude <= 5 -- [FIX PERF 60b] so sanh float == khong bao gio khop -> step khong bao gio xong
              if ((Root.Position - CFrame.new(-1836, 11, 1714).Position).Magnitude <= 5) then notween(CFrame.new(-1836, 11, 1714))end -- [FIX PERF 60b]
      ...
      if _G.Auto_Yama then
	    ...
	    elseif replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress") > 30 then
	      ...
	      if (workspace.Map.Waterfall.SealedKatana.Handle.Position-plr.Character.HumanoidRootPart.Position).Magnitude >= 20 then
            ...
            if zx then
              repeat __DIO_wait(Sec) Attack.Kill(zx,_G.Auto_Yama) until zx.Humanoid.Health <= 0 or not zx.Parent or not _G.Auto_Yama               
...
spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 62] 0.2 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        pcall(function()
            if Quest1 == true then 
	    ...
	    if GetM("Bones") >= 500 and GetM("Ectoplasm") >= 250 and GetM("Dark Fragment") >= 1 then
		  ...
		  elseif GetM("Dark Fragment") < 1 then
		    if _G.AutoMatSoul and World2 then
		      ...
		      if black then repeat __DIO_wait(Sec) Attack.Kill(black, _G.AutoMatSoul)until _G.AutoMatSoul or black.Humanoid.Health <= 0
                    ...
                    if (CFramePos.Position - Root.Position).Magnitude <= 1000 and workspace.Enemies:FindFirstChild(NameMonHaze) then
                      for i,v in pairs(workspace.Enemies:GetChildren()) do
                        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v:FindFirstChild("Humanoid").Health > 0 and v:FindFirstChild("HazeESP") then
                          repeat __DIO_wait(Sec) Attack.Kill(v, _G.CDK_YM) until not _G.CDK_YM or tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == 2 or not v:FindFirstChild("HazeESP") or v.Humanoid.Health <= 0
                        end
                      end
                    else   
                      _tp(CFramePos)                               
                ...
                if (Root.Position - workspace.Map.HellDimension.Spawn.Position).Magnitude <= 1000 then
                  for gg,ez in pairs(workspace.Map.HellDimension.Exit:GetChildren()) do
                    if tonumber(gg) == 2 then
                      repeat __DIO_wait(0.15) Root.CFrame -- [FIX SPAM-9] naked wait -> 0.15s = workspace.Map.HellDimension.Exit.CFrame until not _G.CDK_YM or tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) == 3
                    end
                  end
                  EquipWeapon(_G.SelectWeapon)
                  if tonumber(replicated.Remotes.CommF_:InvokeServer("CDKQuest","Progress")["Evil"]) ~= 3 then
    ...
    if _G.Greybeard then
      pcall(function()
        ...
        elseif GetWP("Bisento") then
          ...
          if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Greybeard)until _G.Greybeard == false or not v.Parent or v.Humanoid.Health <= 0
  ...
  while __DIO_wait(1) do -- [FIX PERF 62] 0.3 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    if _G.WardenBoss then
      pcall(function()
        ...
        if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.WardenBoss) until _G.WardenBoss == false or not v.Parent or v.Humanoid.Health <= 0 
  ...
  while __DIO_wait(1) do -- [FIX PERF 62] 0.3 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    if _G.MarinesCoat then
      pcall(function()
        ...
        if v then repeat __DIO_wait(Sec) Attack.Kill(v, _G.MarinesCoat) until _G.MarinesCoat == false or not v.Parent or v.Humanoid.Health <= 0
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.3 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    if _G.BlackSpikey then
      pcall(function()
        local v = GetConnectionEnemies("Jeremy")
        if v then repeat __DIO_wait(Sec) Attack.Kill(v, _G.BlackSpikey)until _G.BlackSpikey == false or not v.Parent or v.Humanoid.Health <= 0
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.35 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    if _G.Auto_SwanGG then
      pcall(function()
        local v = GetConnectionEnemies("Don Swan")
        if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Auto_SwanGG)until _G.Auto_SwanGG == false or not v.Parent or v.Humanoid.Health <= 0
  ...
  while __DIO_wait(Sec) do
    pcall(function()
      if _G.Auto_Cavender then
	    ...
	    if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Auto_Cavender)until not _G.Auto_Cavender or v.Humanoid.Health <= 0
...
spawn(function()
  while __DIO_wait(Sec) do
    if _G.AutoSerpentBow then
      ...
      if v then	repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoSerpentBow)until not _G.AutoSerpentBow or not v.Parent or v.Humanoid.Health <= 0
      ...
      if _G.Auto_Rainbow_Haki then
        if plr.PlayerGui.Main.Quest.Visible == false then
          if _G.GetQFast then
            ...
            if (plr.Character.HumanoidRootPart.Position - Rainbow1.Position).Magnitude > 5 then -- [FIX PERF 24] CFrame== hang so khong bao gio khop -> step "Bet" khong bao gio chay
              _tp(Rainbow1)
          ...
          elseif plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Hydra Leader") then
            ...
            if v then
              ...
              if (plr.Character.HumanoidRootPart.Position - framelong1).Magnitude <= 5 then _tp(framelong2) end -- [FIX PERF 25] so sanh Vector3 == hang so khong bao gio khop
            end
          elseif plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Kilo Admiral") then
            ...
            elseif plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Captain Elephant") then
              ...
              if (plr.Character.HumanoidRootPart.Position - gamergayror1).Magnitude > 5 then -- [FIX PERF 26] sau khi entrance dua toi, Position khong bao gio == hang so -> spam requestEntrance 4 lan/s
  ...
  while __DIO_wait(Sec) do
    if _G.AutoKenVTWO then
	  ...
	  if plr.PlayerGui.Main.Quest.Visible == true and string.find(plr.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text,"Defeat 50 Forest Pirates") then
	    local v = GetConnectionEnemies("Forest Pirate")
        if v then
	      repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoKenVTWO) until not _G.AutoKenVTWO or v.Humanoid.Health <= 0 or plr.PlayerGui.Main.Quest.Visible == false
	    else
	      _tp(Kv2Pos4)
	    end
	  elseif plr.PlayerGui.Main.Quest.Visible == true then 
	  ...
	  if not GetBP("Fruit Bowl") then   	    
	    ...
	    if plr.Backpack:FindFirstChild("Banana") and plr.Backpack:FindFirstChild("Apple") and plr.Backpack:FindFirstChild("Pineapple") or plr:FindFirstChild("Banana") and plr:FindFirstChild("Apple") and plr:FindFirstChild("Pineapple") then
	      repeat wait() _tp(Kv2Pos1) until _G.AutoKenVTWO or (plr.Character.HumanoidRootPart.Position - Kv2Pos1.Position).Magnitude <= 5 -- [FIX PERF 27] CFrame== hang so khong bao gio thoat repeat
	      ...
	      if plr.Backpack:FindFirstChild("Fruit Bowl") or plr.Character:FindFirstChild("Fruit Bowl") then
		    ...
		    elseif (plr.Character.HumanoidRootPart.Position - Kv2Pos3.Position).Magnitude <= 5 then -- [FIX PERF 27b]
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.8 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
    ...
    if _G.DummyMan then
      pcall(function()
        if plr.PlayerGui.Main.Quest.Visible == false then	
          ...
          if v then
		    repeat __DIO_wait(Sec) Attack.Kill(v,_G.DummyMan) until not _G.DummyMan or not v.Parent or v.Humanoid.Health <= 0
	      else
	  ...
	  local M_Frag = plr.Data.Fragments.Value
        if plr:FindFirstChild("WeaponAssetCache") then
          if not GetBP("Superhuman") then                    
            ...
            if (M_Beli >= 150000) and __DIO_FlyTo(__DIO_MeleeCF("BuyBlackLeg"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyBlackLeg") end -- [FIX SYNC-AUTO] bay den teacher (CF user) roi moi mua
            ...
            if (M_Beli >= 500000) and __DIO_FlyTo(__DIO_MeleeCF("BuyElectro"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyElectro") end -- [FIX SYNC-AUTO]
            ...
            if (M_Beli >= 750000) and __DIO_FlyTo(__DIO_MeleeCF("BuyFishmanKarate"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyFishmanKarate") end -- [FIX SYNC-AUTO]
            ...
            if (M_Frag >= 1500) and __DIO_FlyTo(__DIO_MeleeCF("BlackbeardReward"), 15) then replicated.Remotes.CommF_:InvokeServer("BlackbeardReward","DragonClaw","2") end -- [FIX SYNC-AUTO]
      ...
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not GetBP("Death Step") then          
            if not GetBP("Black Leg") then if __DIO_FlyTo(__DIO_MeleeCF("BuyBlackLeg"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyBlackLeg") end end -- [FIX SYNC-AUTO]
            if GetBP("Black Leg") and GetBP("Black Leg").Level.Value >= 400 then if __DIO_FlyTo(__DIO_MeleeCF("BuyDeathStep"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyDeathStep") end _G.Level = false elseif GetBP("Black Leg") and GetBP("Black Leg").Level.Value < 399 then _G.Level = true end
            ...
            if workspace.Map.IceCastle.Hall.LibraryDoor.PhoeyuDoor.Transparency == 0 then            
              if GetBP("Library Key") then if GetBP("Library Key") then if __DIO_FlyTo(CFrame.new(6371.2001953125, 296.63433837890625, -6841.18115234375), 5) then replicated.Remotes.CommF_:InvokeServer("BuyDeathStep") end end -- [FIX SYNC-AUTO] bay that (tween truc tiep HRP thay repeat-_tp tren block) + den noi (<=5) moi mua; giu nguyen dieu kien PhoeyuDoor + Library Key
-- [FIX SYNC-AUTO] (dong guard cu da thay bang FlyTo ben tren)
		        elseif not GetBP("Library Key") then
		          local v = GetConnectionEnemies("Awakened Ice Admiral")
		          if v then	repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoDeathStep) until not v.Parent or v.Humanoid.Health <= 0 or _G.AutoDeathStep == false or GetBP("Library Key") or GetBP("Death Step")
	              else __DIO_FlyTo(CFrame.new(5668.9780273438, 28.519989013672, -6483.3520507813), 10) -- [FIX SYNC-AUTO] bay that toi vung hunt Ice Admiral
	              end
		        end		    
              end
      ...
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then  
          if not GetBP("Sharkman Karate") then          
            if not GetBP("Fishman Karate") then if __DIO_FlyTo(__DIO_MeleeCF("BuyFishmanKarate"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyFishmanKarate") end end -- [FIX SYNC-AUTO]
            if GetBP("Fishman Karate") and GetBP("Fishman Karate").Level.Value >= 400 then if __DIO_FlyTo(__DIO_MeleeCF("BuySharkmanKarate"), 15) then replicated.Remotes.CommF_:InvokeServer("BuySharkmanKarate") end _G.Level = false elseif GetBP("Fishman Karate") and GetBP("Fishman Karate").Level.Value < 399 then _G.Level = true end
            if GetBP("Fishman Karate") or GetBP("Fishman Karate").Level.Value >= 400 then           
              if GetBP("Water Key") then
		        if string.find(replicated.Remotes.CommF_:InvokeServer("BuySharkmanKarate"), "keys") then  
			      if GetBP("Water Key") then
			        if __DIO_FlyTo(CFrame.new(-2604.6958, 239.432526, -10315.1982, 0.0425701365, 0, -0.999093413, 0, 1, 0, 0.999093413, 0, 0.0425701365), 5) then -- [FIX SYNC-AUTO] bay that + den noi moi mua; thay repeat-_tp + so sanh Position == float chinh xac (khong bao gio khop -> buoc mua chet sau repeat)
	                replicated.Remotes.CommF_:InvokeServer("BuySharkmanKarate")
                    end -- [FIX SYNC-AUTO] dong if FlyTo
		          end
		        end
		      elseif not GetBP("Water Key") then
		        local v = GetConnectionEnemies("Tide Keeper")
		        if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Auto_SharkMan_Karate)until not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_SharkMan_Karate == false or GetBP("Water Key") or GetBP("Sharkman Karate")		
	            else __DIO_FlyTo(CFrame.new(-3053.9814453125, 237.18954467773, -10145.0390625), 10) -- [FIX SYNC-AUTO] bay that toi vung hunt Tide Keeper
	            end
		      end		                  
            end          
          end
...
spawn(function()
  while __DIO_wait(Sec) do
    if _G.Auto_Electric_Claw then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not GetBP("Electro") then if __DIO_FlyTo(__DIO_MeleeCF("BuyElectro"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyElectro") end end -- [FIX SYNC-AUTO]        
          if GetBP("Electro") and GetBP("Electro").Level.Value >= 400 then
            if replicated.Remotes.CommF_:InvokeServer("BuyElectricClaw", "Start") == nil then __DIO_FlyTo(CFrame.new(-12548, 337, -7481), 10) end -- [FIX SYNC-AUTO] bay that thay notween (instant)
            if __DIO_FlyTo(__DIO_MeleeCF("BuyElectricClaw"), 15) then replicated.Remotes.CommF_:InvokeServer("BuyElectricClaw") end -- [FIX SYNC-AUTO] den Previous Hero (CF user) roi moi mua
...
spawn(function()
  while __DIO_wait(Sec) do
    if _G.AutoDragonTalon then
      pcall(function()
        if plr:FindFirstChild("WeaponAssetCache") then 
        if not GetBP("Dragon Claw") then if __DIO_FlyTo(__DIO_MeleeCF("BlackbeardReward"), 15) then replicated.Remotes.CommF_:InvokeServer("BlackbeardReward","DragonClaw","2") end end -- [FIX SYNC-AUTO]        
          if GetBP("Dragon Claw") and GetBP("Dragon Claw").Level.Value >= 400 then if __DIO_FlyTo(__DIO_MeleeCF("BuyDragonTalon"), 15) then replicated.Remotes.CommF_:InvokeServer("Bones","Buy",1,1) replicated.Remotes.CommF_:InvokeServer("BuyDragonTalon") end -- [FIX SYNC-AUTO] bay den Uzoth (CF user) roi moi doi Bones + mua
          ...
          if GetM("Vampire Fang") >= 20 and GetM("Demonic Wisp") <= 19 then
           ...
           if GetM("Vampire Fang") >= 20 and GetM("Demonic Wisp") >= 20 and GetM("Dark Fragment") <= 1 then
             if World2 then
		       ...
		       if n then repeat __DIO_wait(Sec) Attack.Kill(black,_G.Snaguine) until _G.Snaguine or black.Humanoid.Health <= 0 or not black.Parent
		      else _tp(CFrame.new(3798.4575195313, 13.826690673828, -3399.806640625))
		      end
		    else replicated.Remotes.CommF_:InvokeServer("TravelDressrosa")
	        end
...
spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 62] 0.9 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
...
pcall(UpdateIslandMirageESP) -- [FIX PERF 28] pcall chong thread chet khi nhan vat respawn
                    __DIO_wait(1)
                end
            end)
        else
...
-- [FIX MISSING-FN] UpdateIslandMirageESP: ban cu goi ham nay (vong Esp Mirage + nhanh else don dep)
-- nhung KHONG DUNG DINH NGHIA -> Esp Mirage khong bao gio chay (pcall nuot loi nil). Ham tao 1
-- BillboardGui duy nhat gan dao Mirage va tu cap nhat khoang cach; khi tat toggle thi destroy.
...
task.spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 62] 0.1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        if LookMV3 then
            pcall(MoveCamtoMoon) -- [FIX PERF 42] pcall chong thread chet khi respawn
            replicated.Remotes.CommE:FireServer("ActivateAbility")            
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.6 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
        ...
        if replicated.Remotes.CommF_:InvokeServer("Alchemist","1") ~= 2 then
          ...
          elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
            ...
            elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
              local v = GetConnectionEnemies("Swan Pirate")
              if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Auto_Mink) until GetBP("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Mink == false
    ...
    pcall(function()
      if _G.Auto_Human then
		...
		elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 1 then
		  ...
		  elseif not plr.Backpack:FindFirstChild("Flower 3") and not plr.Character:FindFirstChild("Flower 3") then
		    local v = GetConnectionEnemies("Swan Pirate")
            if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.Auto_Human) until plr.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Auto_Human == false
		    else _tp(CFrame.new(980.0985107421875, 121.331298828125, 1287.2093505859375))end
		  end
		  elseif replicated.Remotes.CommF_:InvokeServer("Alchemist","1") == 2 then
		    replicated.Remotes.CommF_:InvokeServer("Alchemist","3")
...
spawn(function()
    local __prevTier = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
    pcall(function()
        while __DIO_wait(1.2) do -- [FIX FPS 2] 0.2s -> 1.2s
      ...
      if _G.AcientOne then
	    ...
	    for i=1,#BonesTable do
	      ...
	      elseif plr.Character:FindFirstChild("RaceTransformed").Value == false then
	        ...
	        if v then repeat __DIO_wait(Sec) Attack.Kill(v, _G.AcientOne) until _G.AcientOne == false or v.Humanoid.Health <= 0 or not v.Parent
		    else _tp(CFrame.new(-9495.6806640625, 453.58624267578125, 5977.3486328125)) 
		    end
	      end
        end
...
Callback = function()
        ...
        task.delay(2, function()
            __DIO_FlyTo(pos2, 15) -- [FIX FLYBUSY-BTN 1/2] qua busy-wrapper: stomper khong stomp trong luc bay (truoc day tween bi Cancel -> dung yen o pos1)
        end)
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] .1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    pcall(function()
      if _G.Complete_Trials then
	    if tostring(plr.Data.Race.Value) == "Human" or tostring(plr.Data.Race.Value) == "Ghoul" then	      
          ...
          if v then repeat __DIO_wait(Sec) Attack.Kill(v, _G.Complete_Trials)until _G.Complete_Trials == false or not v.Parent or v.Humanoid.Health <= 0 end		
        end
      end
    end)
  end
...
spawn(function()
  while __DIO_wait(Sec) do
    if _G.AutoFireFlowers then
      ...
      local v = GetConnectionEnemies("Forest Pirate")
      if v then repeat __DIO_wait(Sec) Attack.Kill(v,_G.AutoFireFlowers) until not _G.AutoFireFlowers or not v.Parent or v.Humanoid.Health <= 0 or FireFlower
      ...
      if _G.TrainDrago then
	    ...
	    for i=1,#DragoM do
	      ...
	      elseif plr.Character:FindFirstChild("RaceTransformed").Value == false then
	        ...
	        if v then repeat __DIO_wait(Sec) Attack.Kill(v, _G.TrainDrago) until _G.TrainDrago == false or v.Humanoid.Health <= 0 or not v.Parent                    		
...
spawn(function()
  while __DIO_wait(Sec) do
    if _G.DT_Uzoth then
      -- [FIX UZOTH] nil-guard + pcall: plr.Character/HumanoidRootPart nil khi chet/respawn
      -- lam CHET thread -> toggle Upgrade Dragon Talon lac hong. Gio bao an toan.
      pcall(function()
...
spawn(function()
    while __DIO_wait(1.1) do -- [FIX FPS 2] 0.2s -> 1.1s
        if workspace.Map:FindFirstChild("PrehistoricIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
...
spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 38] 0.3 -> 1: GetDescendants ca island nang, quet 1 lan/s du
...
game:GetService("RunService").RenderStepped:Connect(function()
    if getgenv().SpeedBoat then
        ...
        __nextSpeedBoat = tick() + 0.25 -- [FIX PERF 32] set MaxSpeed khong can 60 lan/s; 4 lan/s du
        local plr = game:GetService("Players").LocalPlayer
    ...
    while __DIO_wait(1) do -- [FIX PERF 62] 0.1 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
        pcall(function()
            if getgenv().AutoPressW then
                ...
                local humanoid = __chW and __chW:FindFirstChild("Humanoid") -- [FIX PERF 33] WaitForChild trong loop = churn; FindFirstChild du
...
spawn(function()while __DIO_wait(0.5)do pcall(function() -- [FIX PERF 11] Sec(0.1) -> 0.5 + khong co thuyen thi bo qua: truoc day quet GetDescendants TAT CA thuyen 10 lan/s VA ghi CanCollide=true len moi part ke ca khi tat het toggle
...
spawn(function()
    while __DIO_wait(1.3) do -- [FIX FPS 2] 0.2s -> 1.3s
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.4 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
...
spawn(function()
    pcall(function()
        while __DIO_wait(1.4) do -- [FIX FPS 2] 0.2s -> 1.4s
...
spawn(function()
    local __prevSpy = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
    while __DIO_wait(4) do -- [FIX SPAM-6] 1.5s -> 4s + guard sea3: InfoLeviathan chi ton tai o sea3
        if not sea3 then task.wait(10) end -- skip hoan toan khi khong o sea3
        pcall(function()
...
task.spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 10] 0.5 -> 1.0: moi vong quet 2 lan Workspace:GetDescendants() (rat nang) -> 1 lan/s du
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while BerryEsp do
pcall(berriesEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
    end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while PlayerEsp do
pcall(EspPly) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while ChestESP do
pcall(ChestEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
pcall(DevEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while IslandESP do
pcall(LocationEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while FlowerESP do
pcall(flowerEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while LegenS do
pcall(LegenSword) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while ColorEsp do
pcall(HakiClorEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while ESPGear do
pcall(gearEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
pcall(EventIslandEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
Tabs.Esp:AddToggle({
    ...
    Callback = function(Value)
        ...
        if not Value then
            ...
            task.spawn(function()
                while advanEsp do
pcall(AdvanFruitEsp) -- [FIX PERF 28] pcall: helper dung plr.Character.Head -> nhan vat chet la thread ESP chet im lang
                    __DIO_wait(0.4) -- [FIX FPS 4] moi frame -> 0.4s
                end
            end)
        end
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 52a] Sec -> 1 remote mua fruit ngau nhien 4 lan/s thanh 1 lan/s
   	pcall(function()
...
spawn(function()
    pcall(function()
        while __DIO_wait(1) do -- [FIX PERF 34] 0.1 -> 1s: 2 remote MUA 20 lan/s = spam server; mua 1 lan/s van snipe duoc
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 51] Sec -> 1 2 remote Check+Awaken 8 lan/s thanh 2 lan/s
    pcall(function()
...
Tabs.Raids:AddToggle({
    ...
    Callback = function(Value)
        _G.TpLab = Value
        StopTween(_G.TpLab)
        task.spawn(function() -- [FIX PERF 66] while ngay trong callback BAM thread UI (khong the tat bang cach khac); spawn de no tu chay rieng
        while _G.TpLab do
            __DIO_wait(0.5) -- [FIX PERF 66] wait() moi frame -> 0.5s (topos tu giu vi tri lien tuc nen du)
            if _G.TpLab then
        ...
        end) -- [FIX PERF 66] ket thuc task.spawn
    end
})
...
spawn(function()
    local __prevP = nil -- [FIX PERF 31] SetDesc chi khi gia tri DOI (ui khong ve lai voi cung chuoi)
        while __DIO_wait(1) do -- [FIX FPS 2] 0.1s -> 1s: dem player khong can 10 lan/s
        pcall(function()
            local playerCount = #game:GetService("Players"):GetPlayers()
            local __v = "All Players : " .. playerCount .. (playerCount == 12 and " / 12 [Max]" or " / 12")
            if __v ~= __prevP then __prevP = __v __indexPlayer:SetDesc(__v) end
            if false then
...
Tabs.Combat:AddToggle({
    ...
    Callback = function(Value)
        ...
        spawn(function()
            repeat
                __DIO_wait(0.1)
                local __sp = game:GetService("Players"):FindFirstChild(_G.PlayersList) -- [FIX PERF 29] target doi man hinh/chet -> Character/Humanoid nil lam chet thread spectate va mat cleanup camera
...
local flySpeed = 50
local flyConnection
-- [FIX BUG-4 HIGH] descendantAddedConnection phai nam o scope ngoai de disconnect duoc
-- khi toggleFly(false). Ban cu la local trong if-block -> bi shadow, khong disconnect
-- -> leak 1 DescendantAdded connection moi lan bat fly ON.
...
local function toggleFly(value)
    ...
    if flying then
        ...
        -- [FIX BUG-4] Dung bien outer-scope __DIO_FlyDescConn de disconnect duoc khi OFF
        ...
        -- [FIX BUG-4] Disconnect DescendantAdded connection khi tat fly
        if __DIO_FlyDescConn then __DIO_FlyDescConn:Disconnect() __DIO_FlyDescConn = nil end
...
spawn(function()
    while __DIO_wait(1) do -- [FIX PERF 46] 0.2 -> 1: vong chay MAI MAI ca khi TAT (nhanh else); giam churn x5
        pcall(function()
            ...
            elseif __agilChar and __agilChar:FindFirstChild("Agility") then -- [FIX PERF 46] nil-guard: truoc day index ["Agility"] thuong -> pcall bao loi 5 lan/s khi toggle OFF
...
FindMeleeNPC = function(npcNames)
    -- [FIX BUY-MELEE] TIM NPC tren map: duyet workspace.NPCs (cac NPC
    -- dang dung tren server hien tai - cung cach tim ma GetQuestPointFromNPC
    -- cua source da dung), tra ve model co HumanoidRootPart de lay CFrame.
...
-- [FIX BUY-MELEE 1/2] Bang toa do dung cua 11 teacher theo tung Sea — do USER cung cap truc tiep (nguon: user 03-09-2026, khong phai tu mang).
-- Yeu cau USER: nut Buy Melee phai TWEEN den dung vi tri teacher (chon CF theo sea hien tai) xong MOI goi InvokeServer mua.
-- Key = tham so dau tien cua lenh mua (voi Dragon Claw, lenh mua la "BlackbeardReward","DragonClaw","2" tai Sabi nen key la "BlackbeardReward").
local __DIO_BUY_MELEE_CF = {
    ["BuyBlackLeg"] = { -- Black Leg v1
    ...
    ["BuyFishmanKarate"] = { -- Fishman Karate v1
    ...
    ["BlackbeardReward"] = { -- Dragon Claw v1
    ...
    ["BuySharkmanKarate"] = { -- Sharkman Karate (Fishman Karate v2)
    ...
    ["BuyElectricClaw"] = { -- Electric Claw v2
    ...
    ["BuyDragonTalon"] = { -- Dragon Talon v2
...
-- [FIX BUY-MELEE 3/4] Helper di chuyen DUNG that: tween TRUC TIEP len HumanoidRootPart.
-- PHAN TICH LOI (user bao: "chi hien dang bay den con nhan vat khong bay"): _tp cu TWEEN LEN PART "block"
-- (part ao cua he thong farm, L759) - nhan vat chi di theo block khi co toggle farm dang giu OnFarm/loop theo.
-- Nut bam/auto mua dung mot minh -> tween chay tren block, nhan vat dung yen. Helper nay khong qua block,
-- khong dong shouldTween -> di chuyen duoc bat ke dang bat toggle nao hay khong.
__DIO_MeleeCF = function(key)
    local entry = __DIO_BUY_MELEE_CF[key]
    if not entry then return nil end
    return (sea1 and entry.sea1) or (sea2 and entry.sea2) or entry.sea3
end
-- [FIX FLYTO-REVERT 1/1] BO co che tween rieng + neo (user: "lay tween va source dang dung, bo tween cam giat").
-- PHAN TICH LOI "gan den dich thi bi tween lai cho cu": vong goc L769 (OnFarm, bat khi shouldTween=true)
-- tu KEO NHAN VAT THEO BLOCK khi hai ben cach nhau <=200 stud; FlyTo cu tween thang HRP nen khi HRP bay
-- vao vung <=200 voi block cu -> HRP bi keo nguoc ve block = "cho cu". Cung la ly do nut mua cu dung yen:
-- block nam sot xa >200 -> vong nay keo block ve nhan vat, triet tieu tween.
-- CACH SUA DUNG HE SOURCE: dat block CANH NHAN VAT truoc khi bay roi dung chinh _tp (tween block goc)
-- -> vong L769 tu keo nhan vat theo block suot hanh trinh (luon <=200) = bay that bang dung he dang co,
-- khong co co che song song nao de va cham/hut nguoc. KHONG neo anchor, KHONG dong camera.
__DIO_FlyTo = function(targetCF, arriveDist)
    ...
    if __hum and __hum.Sit then __hum.Sit = false end -- roi ghe de block theo nhan vat bo di
    block.CFrame = __h.CFrame -- [he thong goc] block dung canh nhan vat -> vong L769 keo nhan vat theo block (bay that)
    local __old = shouldTween
    shouldTween = true -- bat OnFarm (L768) de vong keo nhan vat hoat dong; tra lai gia tri cu sau khi xong
    __DIO_FlyBusy = true -- [FIX FLYBUSY 3/3] chan moi vong stomp shouldTween trong suot luc bay
    local __t0 = tick()
    ...
    while tick() - __t0 < __timeout do
        ...
        if not h2 then break end -- mat nhan vat (respawn): thoat, tra lai shouldTween o duoi
        if (h2.Position - targetCF.Position).Magnitude <= arriveDist then
            __ok = true
            break
        end
        shouldTween = true -- lam moi moi nhop (phong-double stomp)
        _tp(targetCF) -- dung dung tween cua source (_tp -> block); gui lai neu mat giua duong
        __DIO_wait(0.25) -- vong phan xa di chuyen - chuan KEEP
    end
    __DIO_FlyBusy = false -- [FIX FLYBUSY] tha co o moi nhanh thoat
    shouldTween = __old
    return __ok
end
BuyMelee = function(npcNames, ...)
    -- [FIX BUY-MELEE 2/2] Khong con tim NPC theo ten (NPC khong load / sai sea -> nut Cam). Chon CF theo sea -> tween -> den noi moi mua.
    local key = (...)
    local buyArgs = { ... } -- goi lai qua unpack trong task.spawn (khong dung ... truc tiep trong ham con)
    local entry = __DIO_BUY_MELEE_CF[key]
    if not entry then
        pcall(function() Window:Notify({Title = "Buy Melee", Content = "Khong co toa do teacher cho: " .. tostring(key), Duration = 3}) end)
        return
    end
    local targetCF = (sea1 and entry.sea1) or (sea2 and entry.sea2) or entry.sea3
    if not targetCF then
        pcall(function() Window:Notify({Title = "Buy Melee", Content = "Khong co toa do cho sea hien tai: " .. tostring(key), Duration = 3}) end)
        return
    end
    task.spawn(function() -- [FIX BUY-MELEE 4/4] khong block UI; bay that bang __DIO_FlyTo (tween truc tiep HRP, khong qua block cua he thong farm)
        local standCF = targetCF * CFrame.new(0, 0, 4) -- dung le 4 stud truoc teacher
        local __char0 = plr.Character
        local __hrp0 = __char0 and __char0:FindFirstChild("HumanoidRootPart")
        if not __hrp0 then return end
        local dist0 = (standCF.Position - __hrp0.Position).Magnitude
        ...
        if __DIO_FlyTo(standCF, 15) then
            replicated.Remotes.CommF_:InvokeServer(unpack(buyArgs)) -- DA DEN NOI MOI GOI MUA (dung args goc cua tung nut)
        else
            pcall(function() Window:Notify({Title = "Buy Melee", Content = "Khong den duoc teacher trong 300s - chua mua. Bam nut lan nua de thu lai.", Duration = 4}) end)
        end
    end)
end
...
Description = "",
Callback = function()
  BuyMelee({"Dark Step Teacher"}, "BuyBlackLeg")
end})
Tabs.Shop:AddButton({
Name = "Buy Electro", 
Description = "",
Callback = function()
  BuyMelee({"Mad Scientist"}, "BuyElectro")
end})
Tabs.Shop:AddButton({
Name = "Buy Fishman Karate", 
Description = "",
Callback = function()
  BuyMelee({"Water Kung Fu Teacher"}, "BuyFishmanKarate")
end})
Tabs.Shop:AddButton({
Name = "Buy DragonClaw", 
Description = "",
Callback = function()
  BuyMelee({"Sabi"}, "BlackbeardReward", "DragonClaw", "2")
end})
Tabs.Shop:AddButton({
Name = "Buy Superhuman", 
Description = "",
Callback = function()
  BuyMelee({"Martial Arts Master"}, "BuySuperhuman")
end})
Tabs.Shop:AddButton({
Name = "Buy Death Step", 
Description = "",
Callback = function()
  BuyMelee({"Phoeyu, the Reformed"}, "BuyDeathStep")
end})
Tabs.Shop:AddButton({
Name = "Buy Sharkman Karate", 
Description = "",
Callback = function()
  BuyMelee({"Daigrock the Sharkman"}, "BuySharkmanKarate")
end})
Tabs.Shop:AddButton({
Name = "Buy ElectricClaw", 
Description = "",
Callback = function()
  BuyMelee({"Previous Hero"}, "BuyElectricClaw")
end})
Tabs.Shop:AddButton({
Name = "Buy DragonTalon", 
Description = "",
Callback = function()
  BuyMelee({"Uzoth"}, "BuyDragonTalon")
end})
Tabs.Shop:AddButton({
Name = "Buy Godhuman", 
Description = "",
Callback = function()
  BuyMelee({"Ancient Monk"}, "BuyGodhuman")
end})
Tabs.Shop:AddButton({
Name = "Buy SanguineArt", 
Description = "",
Callback = function()
  BuyMelee({"Shafi"}, "BuySanguineArt")
end})
...
Tabs.Shop:AddButton({
    Name = "Buy Draco Race",
    Callback = function()
        task.spawn(function() -- [FIX FLYBUSY-BTN 2/2] 3 bug 1 cho: (1) _tp bi stomper Cancel -> repeat-wait cho vo tan = treo UI + khong mua; (2) repeat block UI (bai hoc PERF 66); (3) nguong <1 stud. Gio: bay qua busy-wrapper, den noi (<=3) moi invoke
            if __DIO_FlyTo(CFrame.new(5814.42724609375, 1208.3267822265625, 884.5785522460938), 3) then
                local args = {
                    [1] = {
                        ["NPC"] = "Dragon Wizard",
                        ["Command"] = "DragonRace"
...
-- [FIX MISSING-VAR] 4 bien portal Sea3 dung trong PortalUnLock nhung chua dinh nghia ->
-- Attack.Pos(nil) bao loi trong pcall => "Unlock All Portals" cam tinh. Toa do lay CHINH cac
-- vector requestEntrance da co san trong file (Mansion/Castle/Hydra).
...
Callback = function(Value)
  ...
  for _, __oldcc in ipairs(a:GetChildren()) do -- [FIX PERF 65] ban cu tao 2 effect MOI moi lan bat -> leak; xoa cai cu truoc
  ...
  while _G.RTXMode do __DIO_wait(1) -- [FIX PERF 65] wait() = 60 lan/s set Lighting + FindChild; 1 lan/s du vi Lighting khong tu reset
    ...
    if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") and not plr.Character.HumanoidRootPart:FindFirstChild("PointLight") then -- [FIX PERF 65] nil-guard chet/respawn
...
spawn(function()
  while __DIO_wait(1) do -- [FIX PERF 62] 0.25 -> 1 (yeu cau user: uu tien FPS; vong poll gated nen logic khong time-out)
    if _G.WalkWater then
      pcall(function()
	   ...
	   local upval0 = replicated.Assets.Models.IceSpikes4:Clone()
        ...
        -- [FIX BUG-3 HIGH] :Connect tao 1 permanent signal connection moi giay khi IceWalk bat.
        -- Sau 60s = 60 orphan connections cho GC -> progressive FPS drop.
        -- Fix: :Once tu disconnect sau khi fire, khong tich luy.
...
-- [FIX FAST-V2 1/3] BO IsEntityAlive/GetEnemiesInRange/AttackNoCoolDown (he cu 3 module chong dau nhau)
-- theo script "fast moi" cua user: 1 vong duy nhat. GIU LAI gate cong suat nhung gap = Rate
-- (1 module duy nhat -> slider "Fast Attack Delay" la nguon chuan toc do duy nhat, mac dinh 0.03 ~ 33 don/s).
...
-- [FIX PERF 19] getconnections+getupvalues CHI DOC, khong co tac dung game -> chi chay khi DOI TOOL
-- (truoc: reflection 2 lan/s mien phi); tool moi = ten khac lan truoc.
if a.Name == (__DIO_LastActivedTool or "") then return end
__DIO_LastActivedTool = a.Name -- [FIX PERF 19]
...
task.spawn(function()
  RunSer.Heartbeat:Connect(function()
    pcall(function()      
      ...
      -- [FIX LAG] bản cũ chạy AttackNoCoolDown MỖI FRAME (get_Monster quét toàn bộ Enemies +
      -- getconnections mỗi frame) -> nặng CPU. Giới hạn còn 10 lần/s:
      if tick() - (_G.__DIONoCdLast or 0) < 0.25 then return end -- [FIX PERF 21] 0.1 -> 0.25: cung nhip voi VOLLEY governor (PERF 16), giam ca quet get_Monster/tool
      _G.__DIONoCdLast = tick()
      -- [FIX FAST-V2 2/3] AttackNoCoolDown da bo (he FAST-V2 thay the); vong nay chi con phan auto-click fruit
      ...
      -- [FIX PERF 20] get_Monster quet Enemies+SeaBeasts moi lan goi -> cache 0.3s du dung
...
FastAttackModule = {} -- [SAVE SETTING] GLOBAL: slider "Fast Attack Delay" van dieu khien toc do qua Rate
-- [FIX FAST-V3] Dong bo voi script "fast moi cuc ngan" lan 2 cua user:
--   * Range 75 (truoc 60) · diem hit = HumanoidRootPart (truoc Head-or-HRP) · fire RegisterHit TRUC TIEP
--     (bo duong SendHitsToServer) — dung 1-1 nhu script user da tu test.
--   * GIU 2 thu de khong resurrect benh FPS cu: (a) cap 6 mob/volley — script user khong cap nhung
--     moi diem hit them = server tra them VFX (ly do FPS teo ngay tu dau; muon tang sua so 6 nay);
--     (b) vong __DIO_wait(Rate) duy nhat thay cho RunService.Heartbeat per-frame (60Hz) — sau khi
--     BOOT-FIX, Rate 0.03 chay dung ~33 don/s that, muon nhanh nua keo slider "Fast Attack Delay" xuong 1.
...
__FA_GetTargets = function(playerPos)
    ...
    for i = 1, #enemies do
        ...
        if hum and hrp and hum.Health > 0 then
            if (hrp.Position - playerPos).Magnitude <= 75 then -- [FIX FAST-V3] Range 75 theo script user
                table.insert(targets, {enemy, hrp}) -- [FIX FAST-V3] hit part = HRP theo script user
...
function FastAttackModule.ExecuteFastAttack()
    ...
    __FA_hitEvent:FireServer(targets[1][2], targets) -- [FIX FAST-V3] RegisterHit truc tiep nhu script user
end

-- [FIX FAST-V2] Vong lap DUY NHAT cua he fast attack (thay StartMainLoops 2 vong cu)
task.spawn(function()
    while true do
...
-- [FIX FAST-V2] (StartMainLoops cu da bo - vong duy nhat nam trong khoi FAST-V2 ben tren)
